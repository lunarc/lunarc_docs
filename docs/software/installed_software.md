# Applications installed as modules

## Index
[A](#a) | [B](#b) | [C](#c) | [D](#d) | [E](#e) | [F](#f) | [G](#g) | [H](#h) | [I](#i) | [J](#j) | [K](#k) | [L](#l) | [M](#m) | [N](#n) | [O](#o) | [P](#p) | [Q](#q) | [R](#r) | [S](#s) | [T](#t) | [U](#u) | [V](#v) | [W](#w) | [X](#x) | [Y](#y) | [Z](#z)

## A

| Modulename | Versions |
| :--- | :--- |
| `abaqus` | R2020x, V6R2017x, V6R2019x, V6R2023a, V6R2023x |
| `abc` | 2.2.2 |
| `abc.data` | 1.1 |
| `abe` | 5.1.2 |
| `abind` | 1.4-8 |
| `Abseil` | 20230125.2, 20230125.3, 20240116.1, 20240722.0, 20250512.1 |
| `absl-py` | 1.4.0, 2.1.0, 2.1.0 |
| `acepack` | 1.6.3 |
| `ACTC` | 1.1 |
| `adabag` | 5.1 |
| `ade4` | 1.7-23 |
| `ADGofTest` | 0.3 |
| `admisc` | 0.39 |
| `AdmixTools` | 7.0.2 |
| `AdmixTools2` | 2.0.1 |
| `affine` | 2.4.0 |
| `affxparser` | 1.82.0 |
| `affy` | 1.88.0 |
| `affycoretools` | 1.82.0 |
| `affyio` | 1.80.0 |
| `AFNI` | 24.0.02 |
| `aggregation` | 1.0.1 |
| `AgiMicroRna` | 2.60.0 |
| `agricolae` | 1.3-7 |
| `AICcmodavg` | 2.3-4 |
| `aiobotocore` | 2.15.2 |
| `aiohappyeyeballs` | 2.4.3 |
| `aiohttp` | 3.10.10, 3.10.10, 3.8.5, 3.8.5, 3.9.5 |
| `aioitertools` | 0.12.0 |
| `aiosignal` | 1.3.1 |
| `akima` | 0.6-3.6 |
| `alabama` | 2023.1.0 |
| `alabaster` | 1.0.0 |
| `ALDEx2` | 1.42.0 |
| `AlgDesign` | 1.2.1.2 |
| `Algorithm::Dependency` | 1.112 |
| `Algorithm::Diff` | 1.201 |
| `aliased` | 0.34 |
| `Alien::Base` | 2.83 |
| `Alien::Build::Plugin::Download::GitLab` | 0.01 |
| `Alien::Libxml2` | 0.19 |
| `ALL` | 1.52.0 |
| `alleleCounter` | 4.2.1 |
| `alluvial` | 0.1-2 |
| `AlphaFold` | 2.3.2, 2.3.2-CUDA-12.1.1, 2.3.4-foss-2022a-CUDA-11.7.0-ColabFold |
| `alphafold` | 3.0.0 |
| `alsa-lib` | 1.2.11 |
| `AMAPVox` | 2.2.1 |
| `Amber` | 22.0-AmberTools-22.3, 22.4-AmberTools-22.5-CUDA-11.7.0 |
| `AmberTools` | 22.3 |
| `Amelia` | 1.8.3 |
| `Anaconda3` | 2022.05, 2023.03-1, 2023.07-2, 2023.09-0, 2024.02-1, 2024.06-1 |
| `ANCOMBC` | 2.12.0 |
| `animation` | 2.8 |
| `annaffy` | 1.82.0 |
| `anndata` | 0.10.5.post1, 0.10.5.post1 |
| `annotate` | 1.88.0 |
| `annotated_types` | 0.6.0, 0.7.0 |
| `AnnotationDbi` | 1.72.0 |
| `AnnotationFilter` | 1.34.0 |
| `AnnotationForge` | 1.52.0 |
| `AnnotationHub` | 4.0.0 |
| `ANNOVAR` | 20250302 |
| `ANSYS` | 2023R1, 2025R2 |
| `ansys` | 20.1 |
| `ant` | 1.10.12-Java-11, 1.10.9-Java-11 |
| `ANTLR` | 2.7.7-Java-11 |
| `ANTs` | 2.3.5, 2.5.0 |
| `AnyEvent` | 7.17 |
| `anyio` | 4.3.0, 4.4.0 |
| `anytime` | 0.3.12 |
| `AOCC` | 4.0.0 |
| `AOCL-BLAS` | 5.0, 5.1 |
| `aod` | 1.3.3 |
| `apcluster` | 1.4.14 |
| `ape` | 5.8-1 |
| `apipkg` | 3.0.2 |
| `aplot` | 0.2.9 |
| `App::Cmd` | 0.336, 0.337 |
| `App::cpanminus` | 1.7048 |
| `AppConfig` | 1.71 |
| `appdirs` | 1.4.4 |
| `APR` | 1.7.0 |
| `APR-util` | 1.6.1 |
| `Archive-Zip` | 1.68 |
| `Archive::Extract` | 0.88 |
| `archspec` | 0.1.2, 0.1.3, 0.2.2 |
| `argon2-cffi-bindings` | 21.2.0 |
| `argon2_cffi` | 23.1.0 |
| `argparse` | 2.3.1 |
| `argparse-dataclass` | 2.0.0 |
| `aricode` | 1.0.3 |
| `arm` | 1.14-4 |
| `arm_forge` | 22.1 |
| `Armadillo` | 11.4.3, 12.6.2, 12.8.0, 14.0.3, 14.6.0, 15.0.1 |
| `aroma.affymetrix` | 3.2.3 |
| `aroma.apd` | 0.7.1 |
| `aroma.core` | 3.3.2 |
| `aroma.light` | 3.40.0 |
| `arpack-ng` | 3.8.0, 3.9.0, 3.9.1 |
| `Array::Transpose` | 0.06, 0.07 |
| `Array::Utils` | 0.5 |
| `array_api_compat` | 1.4.1 |
| `arrayhelpers` | 1.1-0 |
| `Arriba` | 2.4.0 |
| `Arrow` | 11.0.0, 14.0.1, 17.0.0, 22.0.0, 6.0.0, 6.0.1, 8.0.0 |
| `arrow-R` | 14.0.1-R-4.3.2, 14.0.1-R-4.4.1, 22.0.0-R-4.5.2, 8.0.0-R-4.2.1 |
| `ASCAT` | 3.2.0-R-4.5.2 |
| `ASE` | 3.22.1, 3.23.0, 3.25.0 |
| `ase` | 3.23.0, 3.25.0 |
| `ase-ext` | 20.9.0 |
| `asgiref` | 3.7.2 |
| `ash` | 1.0-15 |
| `askpass` | 1.2.1 |
| `asn1crypto` | 1.5.1 |
| `asnipe` | 1.1.17 |
| `assertive` | 0.3-6 |
| `assertive.base` | 0.0-9 |
| `assertive.code` | 0.0-4 |
| `assertive.data` | 0.0-3 |
| `assertive.data.uk` | 0.0-2 |
| `assertive.data.us` | 0.0-2 |
| `assertive.datetimes` | 0.0-3 |
| `assertive.files` | 0.0-2 |
| `assertive.matrices` | 0.0-2 |
| `assertive.models` | 0.0-2 |
| `assertive.numbers` | 0.0-2 |
| `assertive.properties` | 0.0-5 |
| `assertive.reflection` | 0.0-5 |
| `assertive.sets` | 0.0-3 |
| `assertive.strings` | 0.0-3 |
| `assertive.types` | 0.0-3 |
| `assertthat` | 0.2.1 |
| `assimp` | 5.3.1, 6.0.2 |
| `assorthead` | 1.4.0 |
| `astor` | 0.8.1 |
| `astropy` | 4.2.1, 5.2.2 |
| `asttokens` | 2.4.1 |
| `astunparse` | 1.6.3 |
| `async-lru` | 2.0.4 |
| `async-timeout` | 4.0.3 |
| `async_generator` | 1.10 |
| `at-spi2-atk` | 2.38.0 |
| `at-spi2-core` | 2.38.0, 2.44.1, 2.49.91, 2.50.0, 2.54.0 |
| `ATACseqQC` | 1.34.0 |
| `ATK` | 2.36.0, 2.38.0 |
| `atomicwrites` | 1.4.1 |
| `attmap` | 0.13.2 |
| `attrdict3` | 2.0.2 |
| `attrs` | 23.2.0, 25.3.0 |
| `AUC` | 0.3.2 |
| `AUCell` | 1.32.0 |
| `audio` | 0.1-11 |
| `audioread` | 3.0.1 |
| `AUGUSTUS` | 3.5.0 |
| `Authen::NTLM` | 1.09 |
| `Authen::SASL` | 2.1700, 2.1800 |
| `Autoconf` | 2.69, 2.71, 2.72 |
| `AutoLoader` | 5.74 |
| `Automake` | 1.16.2, 1.16.3, 1.16.4, 1.16.5, 1.17, 1.18, 1.18.1 |
| `Autotools` | 20200321, 20210128, 20210726, 20220317, 20231222, 20240712, 20250527, 20250626 |
| `av` | 11.0.0 |
| `Avogadro2` | 1.97.0-linux-x86_64 |
| `aws` | 2.5-6 |
| `aws.s3` | 0.3.22 |
| `aws.signature` | 0.6.0 |
| `awsMethods` | 1.1-1 |

## B

| Modulename | Versions |
| :--- | :--- |
| `b3sum` | 1.8.5 |
| `B::COW` | 0.007 |
| `B::Hooks::EndOfScope` | 0.28 |
| `B::Lint` | 1.20 |
| `Babel` | 2.17.0 |
| `babelgene` | 22.9 |
| `backcall` | 0.2.0 |
| `backports` | 1.5.0 |
| `backports.entry-points-selectable` | 1.3.0 |
| `backports.functools-lru-cache` | 2.0.0 |
| `bacr` | 1.0.1 |
| `ballgown` | 2.42.0 |
| `bamtofastq` | 1.4.1 |
| `BamTools` | 2.5.2 |
| `Bandage` | 0.9.0 |
| `bartMachine` | 1.3.4.1 |
| `bartMachineJARs` | 1.2.1 |
| `base64` | 2.0.2 |
| `base64enc` | 0.1-3 |
| `basilisk` | 1.22.0 |
| `basilisk.utils` | 1.22.0 |
| `batchelor` | 1.26.0 |
| `BatchJobs` | 1.10 |
| `batchmeans` | 1.0-4 |
| `batchspawner` | 1.2.0-2a9eda060a875a2b65ca9521368fe052a09c3266 |
| `Battenberg` | 3.0.0-R-4.5.2 |
| `BayesianTools` | 0.1.8 |
| `BayesLogit` | 2.1 |
| `bayesm` | 3.1-6 |
| `BayesPen` | 1.0 |
| `bayesplot` | 1.14.0 |
| `bayestestR` | 0.17.0 |
| `baySeq` | 2.44.0 |
| `Bazel` | 3.7.2, 5.1.1, 6.1.0, 6.3.1, 6.5.0-Java-11 |
| `BB` | 2019.10-1 |
| `BBMap` | 39.19 |
| `BBmisc` | 1.13 |
| `bbmle` | 1.0.25.1 |
| `bcbio-gff` | 0.7.0 |
| `BCEE` | 1.3.2 |
| `BCFtools` | 1.18, 1.19, 1.21 |
| `bcl-convert` | 4.0.3-2el7.x86_64 |
| `bcl2fastq2` | 2.20.0 |
| `bcrypt` | 4.0.1, 4.1.3 |
| `BDgraph` | 2.74 |
| `bdsmatrix` | 1.3-7 |
| `beachmat` | 2.26.0 |
| `beagle-lib` | 3.1.2, 4.0.0, 4.0.1, 4.0.1-CUDA-12.1.1 |
| `beanplot` | 1.3.1 |
| `Beast` | 1.10.4, 1.10.4-CUDA-12.1.1, 2.6.7, 2.7.3, 2.7.7, 2.7.7-CUDA-12.1.1 |
| `BeautifulSoup` | 4.10.0, 4.11.1, 4.12.2, 4.12.3, 4.12.3 |
| `BEDOPS` | 2.4.41 |
| `BEDTools` | 2.30.0, 2.31.0, 2.31.1 |
| `beepr` | 2.0 |
| `beeswarm` | 0.4.0 |
| `beniget` | 0.4.2.post1 |
| `berryFunctions` | 1.22.13 |
| `betareg` | 3.2-4 |
| `bgen` | v1.2.0 |
| `BGEN-enkre` | 1.1.7 |
| `BH` | 1.87.0-1 |
| `BiasedUrn` | 2.0.12 |
| `bibtex` | 0.5.1 |
| `BIEN` | 1.2.7 |
| `bigD` | 0.3.1 |
| `BIGL` | 1.9.3 |
| `biglm` | 0.9-3 |
| `bigmemory` | 4.6.4 |
| `bigmemory.sri` | 0.1.8 |
| `bigsnpr` | R4.2.1, R4.3.2 |
| `BindCraft` | 1.1.0-CUDA-12.1.1, 1.5.1-CUDA-12.1.1 |
| `bindr` | 0.1.2 |
| `bindrcpp` | 0.2.3 |
| `binutils` | 2.34, 2.35, 2.36.1, 2.37, 2.38, 2.39, 2.40, 2.42, 2.44, 2.45 |
| `Bio-DB-HTS` | 3.01 |
| `bio3d` | 2.4-5 |
| `Biobase` | 2.70.0 |
| `BiocBaseUtils` | 1.12.0 |
| `BiocFileCache` | 3.0.0 |
| `BiocGenerics` | 0.56.0 |
| `BiocIO` | 1.20.0 |
| `BiocManager` | 1.30.27 |
| `BiocNeighbors` | 2.4.0 |
| `bioconda` | bio-suite-py3.10-tf, bio-suite-py3.12-torch |
| `BiocParallel` | 1.44.0 |
| `BiocSingular` | 1.26.0 |
| `BiocStyle` | 2.38.0 |
| `BiocVersion` | 3.22.0 |
| `bioframe` | v0.6.4 |
| `biom` | 0.3.12 |
| `biom-format` | 2.1.14 |
| `biomaRt` | 2.66.0 |
| `biomformat` | 1.38.0 |
| `biomod2` | 4.2-6-2 |
| `BioPerl` | 1.7.8 |
| `Biopython` | 1.76-Python-2.7.18, 1.79, 1.81, 1.83, 1.84 |
| `Biostrings` | 2.78.0 |
| `biovizBase` | 1.58.0 |
| `biscuit` | v1.5.0 |
| `Bismark` | 0.24.1 |
| `Bison` | 3.5.3, 3.7.1, 3.7.6, 3.8.2 |
| `bispdep` | 1.0-2 |
| `bit` | 4.6.0 |
| `bit64` | 4.6.0-1 |
| `bitarray` | 3.3.1, 3.6.0 |
| `bitops` | 1.0-9 |
| `bitstring` | 4.3.1 |
| `black` | 24.10.0 |
| `BLAKE3-C` | 1.8.5 |
| `BLAST+` | 2.13.0, 2.14.0, 2.14.1, 2.16.0 |
| `blast_databases` | v5 |
| `BLAT` | 3.7 |
| `blavaan` | 0.5-9 |
| `bleach` | 6.1.0 |
| `Blender` | 4.0.1-linux-x86_64-CUDA-12.1.1 |
| `blender` | 3.5.1, 4.0.2, 4.2.0, 4.5.3, 5.1.2 |
| `blinker` | 1.6.2, 1.8.2 |
| `BLIS` | 0.8.1, 0.9.0, 1.0, 1.1, 2.0 |
| `blme` | 1.0-6 |
| `blob` | 1.2.4 |
| `Blosc` | 1.21.3, 1.21.5 |
| `Blosc2` | 2.6.1, 2.8.0 |
| `bluster` | 1.20.0 |
| `BMA` | 3.18.20 |
| `bmp` | 0.3.1 |
| `bnlearn` | 5.1 |
| `bokeh` | 2.2.3, 2.4.2, 2.4.3, 3.2.1, 3.2.2, 3.4.1, 3.6.0, 3.6.0 |
| `bold` | 1.3.0 |
| `bookdown` | 0.45 |
| `boolean` | 0.46 |
| `Boost` | 1.55.0, 1.74.0, 1.76.0, 1.77.0, 1.79.0, 1.81.0, 1.82.0, 1.83.0, 1.85.0, 1.88.0 |
| `Boost.MPI` | 1.79.0, 1.85.0 |
| `boot` | 1.3-32 |
| `bootstrap` | 2019.6 |
| `Boruta` | 9.0.0 |
| `boto3` | 1.26.163, 1.26.37, 1.35.36, 1.35.36 |
| `botocore` | 1.35.36 |
| `bottleneck` | 1.5.0 |
| `Bowtie` | 1.3.0, 1.3.1 |
| `Bowtie2` | 2.4.5, 2.5.1, 2.5.4 |
| `brainvisa` | v5.1.2-virt |
| `brew` | 1.0-10 |
| `brglm` | 0.7.3 |
| `bridgedist` | 0.1.3 |
| `bridgesampling` | 1.1-2 |
| `brio` | 1.1.5 |
| `brms` | 2.23.0 |
| `Brobdingnag` | 1.2-9 |
| `broom` | 1.0.10 |
| `broom.helpers` | 1.22.0 |
| `broom.mixed` | 0.2.9.6 |
| `Brotli` | 1.0.9, 1.1.0 |
| `Brunsli` | 0.1 |
| `BSgenome` | 1.78.0 |
| `BSgenome.Cfamiliaris.UCSC.canFam3` | 1.4.0 |
| `BSgenome.Hsapiens.UCSC.hg19` | 1.4.3 |
| `BSgenome.Hsapiens.UCSC.hg38` | 1.4.5 |
| `BSgenome.Mmusculus.UCSC.mm10` | 1.4.3 |
| `bslib` | 0.9.0 |
| `bsseq` | 1.46.0 |
| `bst` | 0.3-24 |
| `build` | 1.0.3, 1.0.3, 1.2.2.post1, 1.3.0, 1.3.0 |
| `buildenv` | default, default-CUDA-12.8.0 |
| `bumphunter` | 1.52.0 |
| `BUSCO` | 5.8.2 |
| `Business::ISBN` | 3.009, 3.012 |
| `Business::ISBN::Data` | 20241205.001, 20250801.001 |
| `BWA` | 0.7.17, 0.7.18, 0.7.18-GCC-12.2.0 |
| `bwa-mem2` | 2.2.1 |
| `bwidget` | 1.9.15 |
| `bx-python` | 0.10.0, 0.14.0, 0.14.0, 0.8.13, 0.9.0 |
| `bz2file` | 0.98 |
| `bzip2` | 1.0.8 |

## C

| Modulename | Versions |
| :--- | :--- |
| `ca` | 0.71.1 |
| `CacheControl` | 0.13.1, 0.14.2, 0.14.3 |
| `cachelib` | 0.10.2, 0.13.0 |
| `cachem` | 1.1.0 |
| `cachetools` | 5.3.1, 5.5.0 |
| `cachy` | 0.3.0 |
| `CAGEfightR` | 1.30.0 |
| `CAGEr` | 2.16.0 |
| `Cairo` | 1.7-0 |
| `cairo` | 1.16.0, 1.17.4, 1.17.8, 1.18.0, 1.18.4 |
| `calibrate` | 1.7.7 |
| `callr` | 3.7.6 |
| `CAMERA` | 1.66.0 |
| `Canary::Stability` | 2013 |
| `Capture::Tiny` | 0.48, 0.50 |
| `car` | 3.1-3 |
| `carData` | 3.0-5 |
| `cards` | 0.7.0 |
| `cardx` | 0.3.0 |
| `caret` | 7.0-1 |
| `Carp::Clan` | 6.08 |
| `Carp::Heavy` | 1.50 |
| `CASTEP` | 25.11 |
| `Catch2` | 2.13.10, 2.13.9 |
| `Category` | 2.76.0 |
| `catlearn` | 1.1 |
| `caTools` | 1.18.3 |
| `cattrs` | 23.2.3, 24.1.1 |
| `Cbc` | 2.10.11, 2.10.12, 2.10.5 |
| `CBPS` | 0.23 |
| `ccache` | 4.6.3 |
| `ccdata` | 1.36.0 |
| `CDO` | 2.1.1, 2.4.1, 2.4.4 |
| `celestial` | 1.5.8 |
| `Cellpose` | 2.2.2-CUDA-11.7.0 |
| `CellRanger` | 6.0.0, 7.1.0, 7.2.0, 9.0.0 |
| `cellranger` | 1.1.0 |
| `CellRanger-ARC` | 2.1.0 |
| `CellRanger-ATAC` | 2.1.0, 2.2.0 |
| `Cereal` | 1.3.0 |
| `Ceres-Solver` | 2.2.0 |
| `certifi` | 2023.11.17, 2025.1.31, 2025.4.26, 2025.7.14 |
| `certipy` | 0.1.3 |
| `cffi` | 1.15.1, 1.16.0, 1.17.1, 1.17.1 |
| `cfgv` | 3.4.0 |
| `CFITSIO` | 3.49, 4.2.0, 4.3.0, 4.3.1, 4.4.1, 4.6.2 |
| `CGAL` | 4.14.3, 5.5.2, 5.6, 6.0.1 |
| `cgdsr` | 1.3.0 |
| `CGHbase` | 1.70.0 |
| `CGHcall` | 2.72.0 |
| `cghFLasso` | 0.2-1 |
| `CGI` | 4.66, 4.70 |
| `Cgl` | 0.60.7, 0.60.8 |
| `changepoint` | 2.3 |
| `chardet` | 5.2.0 |
| `CharLS` | 2.4.1 |
| `charset-normalizer` | 3.3.2, 3.4.1, 3.4.2 |
| `charset_normalizer` | 3.4.1, 3.4.2 |
| `Check` | 0.15.2 |
| `checkmate` | 2.3.3 |
| `chemometrics` | 1.4.4 |
| `chex` | 0.1.86 |
| `ChIPpeakAnno` | 3.44.0 |
| `ChIPseeker` | 1.45.0 |
| `chk` | 0.10.0 |
| `chkptstanr` | 0.1.1 |
| `chromVAR` | 1.32.0 |
| `chron` | 2.3-62 |
| `cigarillo` | 1.0.0 |
| `circlize` | 0.4.16 |
| `circular` | 0.5-2 |
| `Clang` | 12.0.1, 13.0.1, 16.0.6, 16.0.6-CUDA-12.1.1, 17.0.6, 18.1.8 |
| `Clang-Python-bindings` | 13.0.1, 16.0.6 |
| `class` | 7.3-23 |
| `Class::Accessor` | 0.51 |
| `Class::Data::Inheritable` | 0.10 |
| `Class::DBI` | v3.0.17 |
| `Class::DBI::SQLite` | 0.11 |
| `Class::Inspector` | 1.36 |
| `Class::ISA` | 0.36 |
| `Class::Load` | 0.25 |
| `Class::Load::XS` | 0.10 |
| `Class::Method::Modifiers` | 2.15 |
| `Class::Singleton` | 1.6 |
| `Class::Tiny` | 1.008 |
| `Class::Trigger` | 0.15 |
| `Class::XSAccessor` | 1.19 |
| `classInt` | 0.4-11 |
| `cld2` | 1.2.6 |
| `cleo` | 2.1.0 |
| `CLHEP` | 2.4.4.0, 2.4.5.3, 2.4.6.2, 2.4.7.1 |
| `cli` | 3.6.5 |
| `click` | 8.1.8, 8.2.1 |
| `click-plugins` | 1.1.1 |
| `cligj` | 0.7.2 |
| `clipr` | 0.8.0 |
| `clisymbols` | 1.2.0 |
| `clock` | 0.7.3 |
| `Clone` | 0.47 |
| `Clone::Choose` | 0.010 |
| `cloudpickle` | 2.2.1, 3.1.1 |
| `Clp` | 1.17.10, 1.17.8, 1.17.9 |
| `clue` | 0.3-66 |
| `cluster` | 2.1.8.1 |
| `clusterGeneration` | 1.3.8 |
| `clusterProfiler` | 4.18.1 |
| `clusterRepro` | 0.9 |
| `clustree` | 0.5.1 |
| `clValid` | 0.7 |
| `CMake` | 3.12.1, 3.18.4, 3.20.1, 3.21.1, 3.22.1, 3.23.1, 3.24.3, 3.26.3, 3.27.6, 3.29.3, 3.31.3, 3.31.8, 4.0.3 |
| `cmna` | 1.0.5 |
| `cmprsk` | 2.2-12 |
| `CMSeq` | 1.0.4 |
| `CNEr` | 1.46.0 |
| `cNORM` | 3.5.1 |
| `CNVkit` | 0.9.10-R-4.4.1 |
| `cobalt` | 4.6.1 |
| `cobs` | 1.3-9-1 |
| `coda` | 0.19-4.1 |
| `codetools` | 0.2-20 |
| `cogapp` | 3.4.1, 3.6.0 |
| `coin` | 1.4-3 |
| `CoinUtils` | 2.11.10, 2.11.12, 2.11.9 |
| `colabdesign` | 1.1.1, 1.1.3 |
| `collapse` | 2.1.4 |
| `COLMAP` | 3.13.0, 3.8 |
| `coloc` | 5.2.3 |
| `colorama` | 0.4.6 |
| `coloredlogs` | 15.0.1 |
| `colorRamps` | 2.3.4 |
| `colorspace` | 2.1-2 |
| `colourpicker` | 1.3.0 |
| `cols4all` | 0.10 |
| `combinat` | 0.0-8 |
| `ComICS` | 1.0.4 |
| `comm` | 0.2.2 |
| `common::sense` | 3.75 |
| `commonmark` | 0.9.1, 2.0.0 |
| `ComplexHeatmap` | 2.26.0 |
| `ComplexUpset` | 1.3.3 |
| `compositions` | 2.0-9 |
| `CompQuadForm` | 1.4.4 |
| `Compress-Raw-Zlib` | 2.202 |
| `Compress::Raw::Bzip2` | 2.213 |
| `Compress::Raw::Zlib` | 2.213 |
| `COMSOL` | 6.2.0.339 |
| `comsol` | 6.1, 6.3, 6.4, 6.4.378 |
| `conan` | 1.58.0, 1.60.2 |
| `concaveman` | 1.2.0 |
| `conda-inject` | 1.3.1, 1.3.2 |
| `conditionz` | 0.1.0 |
| `Config::General` | 2.65, 2.67 |
| `Config::INI` | 0.029 |
| `Config::MVP` | 2.200013 |
| `Config::MVP::Reader::INI` | 2.101465 |
| `Config::Simple` | 4.58 |
| `Config::Tiny` | 2.30 |
| `ConfigArgParse` | 1.7 |
| `configobj` | 5.0.8 |
| `configr` | 0.3.5 |
| `configurable-http-proxy` | 4.5.6, 4.6.1 |
| `conflicted` | 1.2.0 |
| `connection-pool` | 0.0.3 |
| `conquer` | 1.3.3 |
| `ConsensusClusterPlus` | 1.74.0 |
| `ConsRank` | 2.1.5 |
| `Const::Exporter` | v1.2.3, v1.3.0 |
| `Const::Fast` | 0.014 |
| `contextlib2` | 21.6.0 |
| `contfrac` | 1.1-12 |
| `contourpy` | 1.2.1, 1.3.2, 1.3.3 |
| `conumee` | 1.44.0 |
| `Converge` | 5.0.2 |
| `cookiecutter` | 2.6.0 |
| `cooler` | 0.10.2, 0.9.1 |
| `CoordgenLibs` | 3.0.1, 3.0.2 |
| `copCAR` | 2.0-4 |
| `copula` | 1.1-6 |
| `Core/SoftwareTree` | Milan |
| `corpcor` | 1.6.10 |
| `corrplot` | 0.95 |
| `covr` | 3.6.4 |
| `CovSel` | 1.2.2 |
| `covsim` | 1.1.0 |
| `cowplot` | 1.2.0 |
| `coxed` | 0.3.3 |
| `coxme` | 2.2-22 |
| `CP2K` | 2023.1 |
| `cp2k-input-tools` | 0.9.1, 0.9.1 |
| `CPAN::Meta::Check` | 0.018 |
| `CPAN::Meta::Requirements` | 2.143 |
| `CPAN::Requirements::Dynamic` | 0.001, 0.002 |
| `CPAN::Uploader` | 0.103018 |
| `CPANPLUS` | 0.9914, 0.9916 |
| `cpio` | 2.15 |
| `cplm` | 0.7-12.1 |
| `cpp11` | 0.5.2 |
| `cppy` | 1.1.0, 1.2.1, 1.3.1 |
| `crashtest` | 0.4.1 |
| `crayon` | 1.5.3 |
| `credentials` | 2.0.3 |
| `crfsuite` | 0.4.2 |
| `crosstalk` | 1.2.2 |
| `crul` | 1.6.0 |
| `cryolo` | 1.9.9 |
| `Crypt::DES` | 2.07 |
| `Crypt::Rijndael` | 1.16 |
| `cryptography` | 41.0.1, 41.0.5, 42.0.8, 44.0.2, 45.0.5 |
| `cSEM` | 0.6.1 |
| `csSAM` | 1.2.4 |
| `ctmle` | 0.1.2 |
| `cubature` | 2.1.4 |
| `CubeGUI` | 4.8, 4.8.2 |
| `CubeLib` | 4.6, 4.8, 4.8.1, 4.8.2, 4.9 |
| `cubelyr` | 1.0.2 |
| `CubeWriter` | 4.6, 4.8, 4.8.1, 4.8.2, 4.9 |
| `CUDA` | 11.1.1, 11.4.1, 11.7.0, 12.0.0, 12.1.1, 12.2.0, 12.6.0, 12.8.0 |
| `CUDA-Python` | 12.1.0-CUDA-12.1.1 |
| `cuda-python` | 12.1.0 |
| `CUDAcore` | 11.1.1 |
| `cuDNN` | 8.0.4.30-CUDA-11.1.1, 8.4.1.50-CUDA-11.7.0, 8.9.2.26-CUDA-12.1.1, 8.9.2.26-CUDA-12.2.0, 9.5.0.50-CUDA-12.6.0 |
| `CUnit` | 2.1-3 |
| `CuPy` | 13.0.0-foss-2023a-CUDA-12.1.1 |
| `cURL` | 7.69.1, 7.72.0, 7.76.0, 7.78.0, 7.83.0, 7.86.0, 8.0.1, 8.11.1, 8.14.1, 8.3.0, 8.7.1 |
| `curl` | 7.0.0 |
| `cuSPARSELt` | 0.6.0.6-CUDA-12.1.1 |
| `cutadapt` | 4.9, 5.0, 5.0, 5.1, 5.1 |
| `cuTENSOR` | 2.0.1.2-CUDA-12.1.1 |
| `cvAUC` | 1.1.4 |
| `CVST` | 0.2-3 |
| `CVXR` | 1.0-15 |
| `Cwd` | 3.75 |
| `Cwd::Guard` | 0.05 |
| `cycler` | 0.12.1 |
| `Cython` | 3.0.10, 3.0.8, 3.1.1, 3.1.2 |
| `cytolib` | 2.22.0 |
| `CytoML` | 2.22.0 |

## D

| Modulename | Versions |
| :--- | :--- |
| `d3Network` | 0.5.2.1 |
| `dada2` | 1.38.0 |
| `dagitty` | 0.3-4 |
| `dask` | 2021.2.0, 2022.1.0, 2022.10.0, 2023.12.1, 2023.7.1, 2023.9.2, 2023.9.2, 2024.5.1, 2024.9.1, 2024.9.1 |
| `dask-jobqueue` | 0.8.2, 0.8.5 |
| `dask-mpi` | 2022.4.0 |
| `data.table` | 1.16.0, 1.17.8 |
| `data.tree` | 1.2.0 |
| `Data::Dump` | 1.25 |
| `Data::Dumper::Concise` | 2.023 |
| `Data::Grove` | 0.08 |
| `Data::OptList` | 0.114 |
| `Data::Section` | 0.200008 |
| `Data::Section::Simple` | 0.07 |
| `Data::Stag` | 0.14 |
| `Data::Types` | 0.17 |
| `Data::UUID` | 1.227 |
| `DataCombine` | 0.2.21 |
| `datawizard` | 1.3.0 |
| `date` | 1.2-42 |
| `Date::Handler` | 1.2 |
| `Date::Language` | 2.33 |
| `DateTime` | 1.65, 1.66 |
| `DateTime::Locale` | 1.44, 1.45 |
| `DateTime::TimeZone` | 2.63, 2.65 |
| `DateTime::Tiny` | 1.07, 1.08 |
| `datrie` | 0.8.2 |
| `dav1d` | 1.5.2 |
| `DB` | 18.1.40 |
| `DB_File` | 1.858 |
| `dbarts` | 0.9-32 |
| `DBD-mysql` | 4.050 |
| `DBD::CSV` | 0.60, 0.62 |
| `DBD::SQLite` | 1.76 |
| `DBI` | 1.2.3, 1.645, 1.647 |
| `DBIx::Admin::CreateTable` | 2.11 |
| `DBIx::Admin::DSNManager` | 2.02 |
| `DBIx::Admin::TableInfo` | 3.04 |
| `DBIx::ContextualFetch` | 1.03 |
| `DBIx::Simple` | 1.37 |
| `dbplyr` | 2.5.1 |
| `dbscan` | 1.2.3 |
| `DBus` | 1.13.18, 1.14.0, 1.15.2, 1.15.4, 1.15.8, 1.16.2 |
| `dbus-glib` | 0.112 |
| `dcurver` | 0.9.3 |
| `ddalpha` | 1.3.16 |
| `ddPCRclust` | 1.30.0 |
| `deal` | 1.2-42 |
| `deap` | 1.4.3 |
| `debugme` | 1.2.0 |
| `debugpy` | 1.8.7 |
| `DECIPHER` | 3.6.0 |
| `decontam` | 1.30.0 |
| `decorator` | 5.2.1 |
| `decoupleR` | 2.16.0 |
| `DeepLabCut` | 2.3.5-CUDA-11.7.0, 2.3.6-CUDA-11.7.0 |
| `deepTools` | 2.5.4, 3.5.2 |
| `DeepVariant` | 1.10.0-virt |
| `defusedxml` | 0.7.1 |
| `DEGseq` | 1.64.0 |
| `DelayedArray` | 0.36.0 |
| `DelayedMatrixStats` | 1.32.0 |
| `deldir` | 2.0-4 |
| `Delly` | 1.1.6 |
| `DeltaLake` | 0.15.1 |
| `dendextend` | 1.19.1 |
| `DendroPy` | 4.5.2, 4.6.1 |
| `densEstBayes` | 1.0-2.2 |
| `DEoptim` | 2.2-8 |
| `DEoptimR` | 1.1-4 |
| `dep-logic` | 0.4.6 |
| `Deprecated` | 1.2.13, 1.2.14 |
| `deprecation` | 2.1.0 |
| `DepthProc` | 2.1.6 |
| `derfinder` | 1.44.0 |
| `derfinderHelper` | 1.44.0 |
| `Deriv` | 4.2.0 |
| `desc` | 1.4.3 |
| `DescTools` | 0.99.60 |
| `DESeq2` | 1.50.0 |
| `deSolve` | 1.40 |
| `Devel::CheckCompiler` | 0.07 |
| `Devel::CheckLib` | 1.16 |
| `Devel::Cycle` | 1.12 |
| `Devel::FindPerl` | 0.016 |
| `Devel::GlobalDestruction` | 0.14 |
| `Devel::OverloadInfo` | 0.007 |
| `Devel::Size` | 0.84, 0.85 |
| `Devel::StackTrace` | 2.05 |
| `devtools` | 2.4.6 |
| `DEXSeq` | 1.56.0 |
| `dfidx` | 0.2-0 |
| `dfoptim` | 2023.1.0 |
| `DFT-D3` | 3.2.0 |
| `DHARMa` | 0.4.7 |
| `dHSIC` | 2.1 |
| `diagram` | 1.6.5 |
| `DiagrammeR` | 1.0.11 |
| `DIAMOND` | 2.1.0, 2.1.11, 2.1.8, 2.1.9 |
| `DiceKriging` | 1.6.1 |
| `dichromat` | 2.0-0.1 |
| `diffcyt` | 1.30.0 |
| `diffobj` | 0.3.6 |
| `digest` | 0.6.37 |
| `Digest::HMAC` | 1.05 |
| `Digest::MD5::File` | 0.08 |
| `Digest::SHA1` | 2.13 |
| `dill` | 0.3.6, 0.3.7, 0.3.9, 0.3.9 |
| `dimRed` | 0.2.7 |
| `diptest` | 0.77-2 |
| `dir.expiry` | 1.18.0 |
| `directlabels` | 2025.6.24 |
| `DirichletMultinomial` | 1.52.0 |
| `DiscriMiner` | 0.1-29 |
| `dismo` | 1.3-16 |
| `Dist::CheckConflicts` | 0.11 |
| `Dist::Zilla` | 6.032, 6.033 |
| `distillery` | 1.2-2 |
| `distlib` | 0.3.9, 0.4.0 |
| `distr` | 2.9.7 |
| `distrEx` | 2.9.6 |
| `distributed` | 2023.9.2, 2024.9.1 |
| `distributional` | 0.5.0 |
| `DistributionUtils` | 0.6-2 |
| `distro` | 1.9.0 |
| `diveRsity` | 1.9.90 |
| `dlm` | 1.1-6.1 |
| `dm-haiku` | 0.0.12, 0.0.12-CUDA-12.1.1 |
| `dm-tree` | 0.1.8 |
| `DMCfun` | 4.0.1 |
| `DMRcaller` | 1.42.0 |
| `DNABarcodes` | 1.40.0 |
| `DNAcopy` | 1.84.0 |
| `dnaio` | 1.2.3 |
| `DO.db` | 2.9 |
| `doBy` | 4.7.0 |
| `doc2vec` | 0.2.0 |
| `docker` | 7.0.0 |
| `docopt` | 0.6.2, 0.7.2 |
| `docrep` | 0.3.2 |
| `docstring` | 1.0.0 |
| `docutils` | 0.21.2 |
| `doit` | 0.36.0 |
| `doMC` | 1.3.8 |
| `doParallel` | 1.0.17 |
| `dorado` | 0.3.1-CUDA-11.7.0, 0.5.1-CUDA-11.7.0, 0.7.1-CUDA-11.7.0, 0.7.3-CUDA-11.7.0 |
| `doRNG` | 1.8.6.2 |
| `DOSE` | 4.4.0 |
| `doSNOW` | 1.0.20 |
| `dotCall64` | 1.2 |
| `double-conversion` | 3.1.5, 3.2.0, 3.2.1, 3.3.0, 3.3.1 |
| `downlit` | 0.4.4 |
| `downloader` | 0.4.1 |
| `Doxygen` | 1.11.0, 1.14.0, 1.8.20, 1.9.1, 1.9.4, 1.9.5, 1.9.7, 1.9.8 |
| `dpath` | 2.1.6, 2.2.0 |
| `dpclust` | v2.2.8 |
| `dplyr` | 1.1.4 |
| `dqrng` | 0.4.1 |
| `dr` | 3.0.11 |
| `dreamerr` | 1.5.0 |
| `drgee` | 1.1.10-3 |
| `dRiftDM` | 0.3.1 |
| `DRIMSeq` | 1.38.0 |
| `DropletUtils` | 1.30.0 |
| `DRR` | 0.0.4 |
| `drugCombo` | 1.2.1 |
| `DSS` | 2.58.0 |
| `DT` | 0.34.0 |
| `dtangle` | 2.0.9 |
| `dtplyr` | 1.3.2 |
| `DTRreg` | 2.3 |
| `dtw` | 1.23-1 |
| `dulwich` | 0.21.7, 0.22.8 |
| `dummies` | 1.5.6 |
| `dune-core` | 2.8.0.post1 |
| `dune-fem` | 2.8.0.6 |
| `dupRadar` | 1.40.0 |
| `dupsifter` | v1.2.0 |
| `DXchange` | 0.1.8 |
| `dygraphs` | 1.1.1.6 |
| `dymola` | 2024x |
| `dynamicTreeCut` | 1.63-1 |
| `DynDoc` | 1.88.0 |

## E

| Modulename | Versions |
| :--- | :--- |
| `e1071` | 1.7-16 |
| `e3nn` | 0.3.3, 0.3.3-CUDA-11.7.0 |
| `earth` | 5.3.4 |
| `EasyABC` | 1.5.2 |
| `EasyBuild` | 4.5.0, 4.6.0, 4.7.0, 4.7.1, 4.7.2, 4.8.0, 4.8.1, 4.8.2, 4.9.0, 4.9.1, 4.9.2, 4.9.3, 4.9.4, 5.0.0, 5.1.0, 5.1.1, 5.1.2, 5.2.0, 5.2.1, 5.3.0 |
| `EBImage` | 4.52.0 |
| `ecBuild` | 3.8.0, 3.8.5 |
| `ecCodes` | 2.22.1, 2.31.0, 2.38.3 |
| `ecdsa` | 0.19.1 |
| `eclipse-ide` | 4.32.0 |
| `ECOSolveR` | 0.5.5 |
| `edgeR` | 4.8.0 |
| `editables` | 0.5 |
| `edlib` | 1.3.9.post1, 1.3.9.post1 |
| `egg` | 0.4.5 |
| `eido` | 0.2.2, 0.2.3 |
| `Eigen` | 3.3.8, 3.3.9, 3.4.0 |
| `elementpath` | 4.1.5 |
| `elfutils` | 0.183, 0.185, 0.187, 0.193 |
| `Elk` | 8.5.2 |
| `ellipse` | 0.5.0 |
| `ellipsis` | 0.3.2 |
| `elliptic` | 1.5-0 |
| `ELPA` | 2021.11.001, 2023.05.001, 2024.05.001, 2025.01.002, 2025.06.001 |
| `Email::Date::Format` | 1.008 |
| `emdbook` | 1.3.14 |
| `emmeans` | 2.0.0 |
| `emoa` | 0.5-3 |
| `emulator` | 1.2-24 |
| `Encode` | 3.21 |
| `Encode::Locale` | 1.05 |
| `energy` | 1.7-12 |
| `ENMeval` | 2.0.5.2 |
| `enrichplot` | 1.30.0 |
| `EnsDb.Hsapiens.v75` | 2.99.0 |
| `EnsDb.Hsapiens.v79` | 2.99.0 |
| `EnsDb.Hsapiens.v86` | 2.99.0 |
| `ensembldb` | 2.34.0 |
| `entropy` | 1.3.2 |
| `ENVI` | 6.0, 6.1, 6.2 |
| `EnvStats` | 3.1.0 |
| `EPICS` | base-7.0.7 |
| `epitools` | 0.5-10.1 |
| `ergm` | 4.10.1 |
| `ergm.count` | 4.1.3 |
| `ergm.multi` | 0.3.0 |
| `Error` | 0.17029, 0.17030 |
| `escape` | 2.6.1 |
| `ESMF` | 8.3.0, 8.6.0 |
| `estimability` | 1.5.1 |
| `et_xmlfile` | 1.1.0, 2.0.0 |
| `etils` | 1.10.0 |
| `Eval::Closure` | 0.14 |
| `evaluate` | 1.0.5 |
| `EValue` | 4.1.4 |
| `evd` | 2.3-7.1 |
| `Exact` | 3.3 |
| `Exception::Class` | 1.45 |
| `exceptiongroup` | 1.1.1, 1.2.2, 1.3.0 |
| `execnet` | 2.0.2, 2.1.1 |
| `executing` | 2.0.1 |
| `Exonerate` | 2.4.0 |
| `expandvars` | 0.12.0 |
| `expat` | 2.2.9, 2.4.1, 2.4.8, 2.4.9, 2.5.0, 2.6.2, 2.6.4, 2.7.1 |
| `Expect` | 1.38 |
| `expecttest` | 0.1.3, 0.1.5, 0.2.1 |
| `ExperimentHub` | 3.0.0 |
| `expm` | 1.0-0 |
| `Exporter::Declare` | 0.114 |
| `Exporter::Tiny` | 1.006002 |
| `ExPosition` | 2.11.0 |
| `expsmooth` | 2.3 |
| `extraDistr` | 1.10.0 |
| `extrafont` | 0.20 |
| `extrafontdb` | 1.1 |
| `extRemes` | 2.2-1 |
| `ExtUtils::CBuilder` | 0.280236 |
| `ExtUtils::Config` | 0.010 |
| `ExtUtils::Constant` | 0.25 |
| `ExtUtils::CppGuess` | 0.27 |
| `ExtUtils::Helpers` | 0.028 |
| `ExtUtils::InstallPaths` | 0.014 |
| `ExtUtils::MakeMaker` | 7.70, 7.76 |
| `ExtUtils::ParseXS` | 3.51, 3.58 |

## F

| Modulename | Versions |
| :--- | :--- |
| `FabIO` | 0.14.0, 2024.9.0 |
| `factoextra` | 1.0.7 |
| `FactoMineR` | 2.12 |
| `FactorCopula` | 0.9.3 |
| `fail` | 1.3 |
| `fANCOVA` | 0.6-1 |
| `fansi` | 1.0.6 |
| `farver` | 2.1.2 |
| `fastahack` | 1.0.0 |
| `fastcluster` | 1.3.0 |
| `fastDummies` | 1.7.5 |
| `fasterize` | 1.1.0 |
| `fastGHQuad` | 1.0.1 |
| `fastICA` | 1.2-7 |
| `fastjsonschema` | 2.19.1, 2.20.0, 2.21.1 |
| `fastlogranktest` | 0.2.1 |
| `fastmap` | 1.2.0 |
| `fastmatch` | 1.1-6 |
| `fastmatrix` | 0.6-6 |
| `fastp` | 0.23.4 |
| `FastQC` | 0.11.9-Java-11 |
| `fastseg` | 1.56.0 |
| `FastTree` | 2.1.11 |
| `fBasics` | 4041.97 |
| `fda` | 6.3.0 |
| `FDb.InfiniumMethylation.hg19` | 2.2.0 |
| `fdrtool` | 1.2.18 |
| `FDS` | 6.7.7, 6.7.9, 6.9.1 |
| `fds` | 1.8 |
| `feather` | 0.3.5 |
| `feature` | 1.2.15 |
| `Fennec::Lite` | 0.004 |
| `fermi-lite` | 20190320 |
| `ff` | 4.5.2 |
| `FFI::CheckLib` | 0.31 |
| `FFmpeg` | 4.3.1, 4.3.2, 4.4.2, 5.1.2, 6.0, 7.0.2, 7.1.1, 7.1.2 |
| `ffnvcodec` | 11.1.5.2, 12.0.16.0, 12.1.14.0, 12.2.72.0, 13.0.19.0 |
| `FFTW` | 3.3.10, 3.3.8, 3.3.9 |
| `fftw` | 1.0-9 |
| `FFTW.MPI` | 3.3.10 |
| `fftwtools` | 0.9-11 |
| `fgsea` | 1.36.0 |
| `fields` | 17.1 |
| `file` | 5.43 |
| `File::chdir` | 0.1011 |
| `File::CheckTree` | 4.42 |
| `File::Copy::Recursive` | 0.45 |
| `File::Copy::Recursive::Reduced` | 0.008 |
| `File::Find::Rule` | 0.34, 0.35 |
| `File::Find::Rule::Perl` | 1.16 |
| `File::Grep` | 0.02 |
| `File::HomeDir` | 1.006 |
| `File::Listing` | 6.16 |
| `File::Next` | 1.18 |
| `File::Path` | 2.18 |
| `File::pushd` | 1.016 |
| `File::Remove` | 1.61 |
| `File::ShareDir` | 1.118 |
| `File::ShareDir::Install` | 0.14 |
| `File::Slurp` | 9999.32 |
| `File::Slurp::Tiny` | 0.004 |
| `File::Slurper` | 0.014 |
| `File::Spec` | 3.75 |
| `File::Temp` | 0.2311 |
| `File::Which` | 1.27 |
| `filehash` | 2.4-6 |
| `filelock` | 1.0.3, 3.17.0, 3.18.0 |
| `filetype` | 1.2.0 |
| `filevercmp` | 20191210 |
| `fillpattern` | 1.0.2 |
| `FinaleMe` | v0.58.1 |
| `finalfit` | 1.1.0 |
| `findpython` | 0.6.1, 0.6.3, 1.0.9 |
| `Fiona` | 1.9.5 |
| `fishMod` | 0.29.2 |
| `fitdistrplus` | 1.2-4 |
| `fixest` | 0.13.2 |
| `FKSUM` | 1.0.1 |
| `FLAC` | 1.3.3, 1.3.4, 1.4.2, 1.5.0 |
| `FLANN` | 1.9.2 |
| `flashClust` | 1.01-2 |
| `Flask` | 2.2.2, 2.3.3, 3.0.3 |
| `flask` | 2.3.3, 3.0.3 |
| `Flask-Cors` | 4.0.0, 5.0.0 |
| `Flask-Session` | 0.5.0, 0.8.0 |
| `flatbuffers` | 1.12.0, 2.0.7, 23.1.4, 23.5.26, 24.3.25 |
| `flatbuffers-python` | 2.0, 23.1.4, 23.5.26, 24.3.25 |
| `Flax` | 0.8.4-CUDA-12.1.1 |
| `flax` | 0.8.4 |
| `flex` | 2.6.4 |
| `flexclust` | 1.5.0 |
| `FlexiBLAS` | 3.0.4, 3.2.0, 3.2.1, 3.3.1, 3.4.4, 3.4.5 |
| `flexmix` | 2.3-20 |
| `flextable` | 0.9.10 |
| `flit` | 3.10.1, 3.10.1, 3.12.0, 3.12.0, 3.9.0 |
| `flit-scm` | 1.7.0 |
| `flit_core` | 3.10.1, 3.12.0 |
| `flook` | 0.8.4 |
| `flowAI` | 1.40.0 |
| `flowClean` | 1.48.0 |
| `flowClust` | 3.48.0 |
| `flowCore` | 2.22.0 |
| `flowDensity` | 1.44.0 |
| `flowFP` | 1.68.0 |
| `flowMerge` | 2.58.0 |
| `flowPeaks` | 1.56.0 |
| `FlowSOM` | 2.18.0 |
| `FlowSorted.Blood.EPIC` | 2.14.0 |
| `FlowSorted.CordBloodCombined.450k` | 1.26.0 |
| `flowStats` | 4.22.0 |
| `flowViz` | 1.74.0 |
| `flowWorkspace` | 4.22.0 |
| `FLTK` | 1.3.7, 1.3.8 |
| `fma` | 2.5 |
| `FME` | 1.3.6.4 |
| `fmri` | 1.9.12.1 |
| `FNN` | 1.1.4.1 |
| `Font::TTF` | 1.06 |
| `fontawesome` | 0.5.3 |
| `fontBitstreamVera` | 0.1.1 |
| `fontconfig` | 2.13.92, 2.13.93, 2.13.94, 2.14.0, 2.14.1, 2.14.2, 2.15.0, 2.16.2, 2.17.0 |
| `fontLiberation` | 0.1.0 |
| `fontquiver` | 0.2.1 |
| `fonttools` | 4.53.1, 4.58.4, 4.58.5 |
| `forcats` | 1.0.1 |
| `FORCE` | 3.10.00-virt, 3.10.04-virt, 3.7.10-virt, 3.7.11-virt, 3.7.12-virt, 3.8.00-virt, 3.8.01-virt |
| `foreach` | 1.5.2 |
| `forecast` | 8.24.0 |
| `foreign` | 0.8-90 |
| `formatR` | 1.14 |
| `Formula` | 1.2-5 |
| `formula.tools` | 1.7.1 |
| `foss` | 2020b, 2021a, 2021b, 2022a, 2022b, 2023a, 2023b, 2024.05, 2024a, 2025a, 2025b |
| `fosscuda` | 2020b |
| `fossil` | 0.4.0 |
| `fpc` | 2.2-13 |
| `fpp` | 0.5 |
| `fracdiff` | 1.5-3 |
| `FragPipe` | 20.0-Java-11 |
| `fragpipe` | 23.0-container, 23.1-container, 24.0-container |
| `FRASER` | 2.6.0 |
| `frbs` | 3.2-0 |
| `freebayes` | 1.3.5, 1.3.7-R-4.3.2 |
| `FreeBindCraft` | 1.0.5 |
| `freeglut` | 3.2.1, 3.2.2, 3.4.0, 3.6.0 |
| `FreeImage` | 3.18.0 |
| `FreeSurfer` | 7.3.2-centos8_x86_64 |
| `freetype` | 2.10.3, 2.10.4, 2.11.0, 2.12.1, 2.13.0, 2.13.2, 2.13.3 |
| `fresh` | 0.2.2 |
| `FriBidi` | 1.0.10, 1.0.12, 1.0.13, 1.0.15, 1.0.16 |
| `frozenlist` | 1.4.0, 1.4.1 |
| `fs` | 1.6.6 |
| `FSL` | 6.0.5.1 |
| `fsom` | 20141119, 20151117 |
| `fsspec` | 2024.9.0, 2025.3.2, 2025.7.0 |
| `furrr` | 0.3.1 |
| `fusioncatcher` | v1.33 |
| `FusionInspector` | v2.8.0 |
| `futile.logger` | 1.4.3 |
| `futile.options` | 1.0.1 |
| `future` | 1.0.0, 1.67.0 |
| `future.apply` | 1.20.0 |

## G

| Modulename | Versions |
| :--- | :--- |
| `gam` | 1.22-6 |
| `GAMESS-US` | 20230930-R2-gompi-2022a |
| `gamlss` | 5.5-0 |
| `gamlss.data` | 6.0-7 |
| `gamlss.dist` | 6.1-1 |
| `gamlss.tr` | 5.1-9 |
| `gamm4` | 0.2-7 |
| `gap` | 1.6 |
| `gap.datasets` | 0.0.6 |
| `gapfill` | 0.9.6-1 |
| `gargle` | 1.6.0 |
| `gast` | 0.6.0 |
| `GATE` | 9.2, 9.4 |
| `GATK` | 4.3.0.0-Java-11, 4.5.0.0-Java-17 |
| `gaussian` | g09 |
| `gaussquad` | 1.0-3 |
| `gbm` | 2.2.2 |
| `gbRd` | 0.4.12 |
| `gc` | 8.2.2 |
| `gc3pie` | v2.68-Py3.10.4 |
| `GCC` | 10.2.0, 10.3.0, 11.2.0, 11.3.0, 12.2.0, 12.3.0, 13.2.0, 13.3.0, 14.2.0, 14.3.0 |
| `GCCcore` | 10.2.0, 10.3.0, 11.2.0, 11.3.0, 12.2.0, 12.3.0, 13.2.0, 13.3.0, 14.2.0, 14.3.0, 15.2.0, 9.3.0 |
| `gcccuda` | 2020b |
| `gclus` | 1.3.3 |
| `gcmr` | 1.0.4 |
| `GConf` | 3.2.6 |
| `gcrma` | 2.82.0 |
| `GCTA` | 1.94.1 |
| `GDAL` | 2.4.4, 3.10.0, 3.11.1, 3.11.3, 3.2.1, 3.4.1, 3.5.0, 3.6.2, 3.7.1, 3.9.0 |
| `gdata` | 3.0.1 |
| `GDB` | 13.2 |
| `gdc-client` | 1.6.0 |
| `gdistance` | 1.6.5 |
| `Gdk-Pixbuf` | 2.40.0, 2.42.10, 2.42.11, 2.42.12, 2.42.6, 2.42.8 |
| `GDRCopy` | 2.1-CUDA-11.1.1, 2.3, 2.3.1, 2.4.1, 2.4.4 |
| `gdsfmt` | 1.46.0 |
| `gdtools` | 0.4.4 |
| `Geant4` | 10.7.1, 11.0.2, 11.1.2, 11.2.2 |
| `Geant4-data` | 11.1, 11.2 |
| `gee` | 4.13-29 |
| `geeM` | 0.10.1 |
| `geepack` | 1.3.13 |
| `geex` | 1.1.1 |
| `geiger` | 2.0.11 |
| `genefilter` | 1.92.0 |
| `geneLenDataBase` | 1.46.0 |
| `GeneNet` | 1.2.17 |
| `geneplotter` | 1.88.0 |
| `generics` | 0.1.4 |
| `GENESIS` | 2.40.0 |
| `GENIE3` | 1.32.0 |
| `genomation` | 1.42.0 |
| `GenomeInfoDb` | 1.46.0 |
| `GenomeInfoDbData` | 1.2.15 |
| `GenomicAlignments` | 1.46.0 |
| `GenomicFeatures` | 1.62.0 |
| `GenomicFiles` | 1.46.0 |
| `GenomicInteractions` | 1.44.0 |
| `GenomicRanges` | 1.62.0 |
| `GenomicScores` | 2.22.0 |
| `genoPlotR` | 0.8.11 |
| `GenSA` | 1.1.14.1 |
| `geojsonsf` | 2.0.3 |
| `GEOmap` | 2.5-11 |
| `geometries` | 0.2.4 |
| `geometry` | 0.5.2 |
| `geopandas` | 0.14.2 |
| `geopy` | 2.4.1 |
| `GEOquery` | 2.78.0 |
| `geoR` | 1.9-6 |
| `GEOS` | 3.10.3, 3.11.1, 3.12.0, 3.12.1, 3.12.2, 3.13.1, 3.9.1 |
| `gert` | 2.1.5 |
| `GET` | 1.0-7 |
| `getopt` | 1.20.4 |
| `Getopt::Long` | 2.58 |
| `Getopt::Long::Descriptive` | 0.115, 0.116 |
| `GetoptLong` | 1.0.5 |
| `GetOrganelle` | 1.7.7.1 |
| `gettext` | 0.21, 0.21.1, 0.22, 0.22.5, 0.24, 0.25 |
| `gfbf` | 2022b, 2023a, 2023b, 2024a, 2025a, 2025b |
| `gflags` | 2.2.2 |
| `gfonts` | 0.2.0 |
| `GGally` | 2.4.0 |
| `ggbeeswarm` | 0.7.2 |
| `ggbio` | 1.58.0 |
| `ggcyto` | 1.38.0 |
| `ggdag` | 0.2.13 |
| `ggdendro` | 0.2.0 |
| `ggdist` | 3.3.3 |
| `ggExtra` | 0.11.0 |
| `ggfan` | 0.1.3 |
| `ggforce` | 0.5.0 |
| `ggformula` | 1.0.0 |
| `ggfun` | 0.2.0 |
| `ggh4x` | 0.3.1 |
| `ggiraph` | 0.9.2 |
| `ggnetwork` | 0.5.14 |
| `ggnewscale` | 0.5.2 |
| `ggpicrust2` | 2.5.2 |
| `ggplot2` | 4.0.0 |
| `ggplotify` | 0.1.3 |
| `ggpointdensity` | 0.2.0 |
| `ggprism` | 1.0.7 |
| `ggpubr` | 0.6.2 |
| `ggraph` | 2.2.2 |
| `ggrastr` | 1.0.2 |
| `ggrepel` | 0.9.6 |
| `ggridges` | 0.5.7 |
| `ggsci` | 4.1.0 |
| `ggseqlogo` | 0.2 |
| `ggsignif` | 0.6.4 |
| `ggstance` | 0.3.7 |
| `ggstats` | 0.11.0 |
| `ggtangle` | 0.0.7 |
| `ggtext` | 0.1.2 |
| `ggthemes` | 5.1.0 |
| `ggtree` | 4.0.1 |
| `ggvenn` | 0.1.19 |
| `ggvis` | 0.4.9 |
| `gh` | 1.5.0, 2.52.0 |
| `Ghostscript` | 10.01.2, 10.02.1, 10.03.1, 10.05.1, 9.53.3, 9.54.0, 9.56.1 |
| `giflib` | 5.2.1, 5.2.2 |
| `GillespieSSA` | 0.6.2 |
| `Git` | 0.42 |
| `git` | 2.28.0-nodocs, 2.32.0-nodocs, 2.33.1-nodocs, 2.36.0-nodocs, 2.38.1-nodocs, 2.41.0-nodocs, 2.42.0, 2.45.1, 2.49.0, 2.50.1 |
| `git-lfs` | 3.5.1 |
| `git2r` | 0.36.2 |
| `gitcreds` | 0.1.2 |
| `gitdb` | 4.0.11, 4.0.9 |
| `GitPython` | 3.1.27, 3.1.27, 3.1.31, 3.1.40, 3.1.42, 3.1.42, 3.1.43, 3.1.43 |
| `GJRM` | 0.2-6.8 |
| `GKlib-METIS` | 5.1.1 |
| `GL2PS` | 1.4.2 |
| `GLAD` | 2.74.0 |
| `glasso` | 1.11 |
| `gld` | 2.6.8 |
| `glew` | 2.1.0, 2.2.0-egl |
| `GLib` | 2.66.1, 2.68.2, 2.69.1, 2.72.1, 2.75.0, 2.77.1, 2.78.1, 2.80.4, 2.85.1, 2.85.3 |
| `GLibmm` | 2.66.4 |
| `Glimma` | 2.20.0 |
| `GLIMPSE` | 2.0.0 |
| `glimpse` | v2.0.0 |
| `gllvm` | 2.0.5 |
| `GLM` | 0.9.9.8 |
| `glmmML` | 1.1.7 |
| `glmmTMB` | 1.1.13 |
| `glmnet` | 4.1-10 |
| `glob2` | 0.7 |
| `GlobalAncova` | 4.28.0 |
| `GlobalArrays` | 5.8, 5.8.1, 5.8.2 |
| `GlobalOptions` | 0.1.2 |
| `globals` | 0.18.0 |
| `globaltest` | 5.64.0 |
| `glog` | 0.6.0, 0.7.1 |
| `GLPK` | 5.0 |
| `glslang-SPIRV` | 15.3.0, 15.4.0 |
| `glue` | 1.8.0 |
| `GMAP-GSNAP` | 2023-02-17 |
| `gmm` | 1.9-1 |
| `gmodels` | 2.19.1 |
| `GMP` | 6.2.0, 6.2.1, 6.3.0 |
| `gmp` | 0.7-5 |
| `gmpy2` | 2.1.2, 2.1.5, 2.2.0 |
| `gmsh` | 4.11.1 |
| `GMT` | 6.2.0 |
| `gmxapi` | 0.4.2 |
| `gnumeric` | 0.7-10 |
| `gnuplot` | 5.4.2, 5.4.4, 5.4.6, 5.4.8, 6.0.3 |
| `GO` | 0.04 |
| `Go` | 1.22.1, 1.25.0 |
| `GO.db` | 3.22.0 |
| `GO::Utils` | 0.15 |
| `GObject-Introspection` | 1.66.1, 1.68.0, 1.72.0, 1.74.0, 1.76.1, 1.78.1, 1.80.1, 1.84.0 |
| `goftest` | 1.2-3 |
| `goldilocks` | 0.4.0 |
| `gomkl` | 2022a, 2023b |
| `gomms` | 1.0 |
| `gompi` | 2020b, 2021a, 2021b, 2022a, 2022b, 2023a, 2023b, 2024.05, 2024a, 2025a, 2025b |
| `gompic` | 2020b |
| `google-auth` | 2.22.0 |
| `google-auth-oauthlib` | 1.0.0 |
| `google-pasta` | 0.2.0 |
| `google_auth` | 2.35.0 |
| `googlebenchmark` | 1.9.4 |
| `googledrive` | 2.1.2 |
| `googlesheets4` | 1.1.2 |
| `googletest` | 1.11.0, 1.12.1, 1.13.0, 1.14.0, 1.15.2, 1.17.0 |
| `GOSemSim` | 2.36.0 |
| `goseq` | 1.62.0 |
| `GOstats` | 2.76.0 |
| `GOTCHA` | 1.0.8 |
| `gower` | 1.0.2 |
| `GPArotation` | 2025.3-1 |
| `GPAW` | 24.1.0 |
| `GPAW-setups` | 0.9.20000 |
| `gperf` | 3.1, 3.3 |
| `gperftools` | 2.10, 2.16, 2.17.2 |
| `gplots` | 3.2.0 |
| `Grace` | 5.1.25 |
| `Graph` | 0.9732, 0.9735 |
| `graph` | 1.88.0 |
| `Graph::ReadWrite` | 2.10 |
| `GraphAligner` | 1.0.20 |
| `Graphene` | 1.10.8 |
| `graphite` | 1.56.0 |
| `graphite2` | 1.3.14 |
| `graphlayouts` | 1.2.2 |
| `Graphviz` | 2.47.2, 5.0.0, 8.1.0 |
| `graphviz-python` | 0.20.1 |
| `Greenlet` | 3.0.3 |
| `grf` | 2.5.0 |
| `gridBase` | 0.4-7 |
| `gridExtra` | 2.3 |
| `gridGraphics` | 0.5-1 |
| `GRIDSS` | 2.13.2-Java-11 |
| `gridtext` | 0.1.5 |
| `grImport2` | 0.3-3 |
| `groff` | 1.22.4, 1.23.0 |
| `GROMACS` | 2019.4, 2021.5, 2021.5-PLUMED-2.8.0, 2022.3-nogmxapi, 2023.1, 2023.1-CUDA-11.7.0, 2023.1-nogmxapi, 2023.3, 2023.3-CUDA-11.7.0, 2024.1, 2024.4-PLUMED-2.9.2 |
| `grpcio` | 1.57.0, 1.57.0, 1.62.1, 1.70.0, 1.70.0 |
| `grpreg` | 3.5.0 |
| `GSA` | 1.03.3 |
| `gsalib` | 2.2.1 |
| `gsDesign` | 3.7.0 |
| `GSEABase` | 1.72.0 |
| `gsignal` | 0.3-7 |
| `GSL` | 2.7, 2.8 |
| `gsl` | 2.1-8 |
| `gsmoothr` | 0.1.7 |
| `gson` | 0.1.0 |
| `gss` | 2.2-9 |
| `GST-plugins-bad` | 1.20.2 |
| `GST-plugins-base` | 1.20.2 |
| `gstat` | 2.1-4 |
| `GStreamer` | 1.20.2 |
| `gsubfn` | 0.7 |
| `GSVA` | 2.4.0 |
| `gsw` | 1.2-0 |
| `gt` | 1.1.0 |
| `gtable` | 0.3.6 |
| `GTK+` | 3.24.23 |
| `GTK2` | 2.24.33 |
| `GTK3` | 3.24.33, 3.24.37, 3.24.39, 3.24.42 |
| `GTK4` | 4.7.0 |
| `gtools` | 3.9.5 |
| `GTS` | 0.7.6 |
| `gtsummary` | 2.4.0 |
| `Gurobi` | 10.0.1, 11.0.0, 9.5.0 |
| `GUTS` | 1.2.6 |
| `Gviz` | 1.54.0 |
| `gviz-api` | 1.10.0 |
| `GWASExactHW` | 1.2 |
| `GWASTools` | 1.56.0 |
| `gWidgets2` | 1.0-10 |
| `gWidgets2tcltk` | 1.0-9 |
| `GxEScanR` | 2.0.2 |
| `gzip` | 1.10, 1.12, 1.13, 1.14 |

## H

| Modulename | Versions |
| :--- | :--- |
| `h11` | 0.14.0 |
| `h2o` | 3.44.0.3 |
| `h5mread` | 1.2.0 |
| `h5py` | 3.1.0, 3.11.0, 3.12.1, 3.2.1, 3.6.0, 3.7.0, 3.8.0, 3.9.0 |
| `hahmmr` | 1.0.0 |
| `hal9001` | 0.4.6 |
| `haldensify` | 0.2.8 |
| `hardhat` | 1.4.2 |
| `HarfBuzz` | 11.2.1, 11.4.1, 2.6.7, 2.8.1, 2.8.2, 4.2.1, 5.3.1, 8.2.2, 9.0.0 |
| `harmony` | 1.2.4 |
| `hash` | 2.2.6.3 |
| `Hash::Merge` | 0.302 |
| `Hash::Objectify` | 0.008 |
| `hatch-docstring-description` | 1.1.1 |
| `hatch-fancy-pypi-readme` | 24.1.0, 25.1.0 |
| `hatch-jupyter-builder` | 0.9.1 |
| `hatch-requirements-txt` | 0.4.1 |
| `hatch-vcs` | 0.4.0, 0.5.0 |
| `hatch_jupyter_builder` | 0.9.1 |
| `hatch_nodejs_version` | 0.3.2 |
| `hatchling` | 1.18.0, 1.24.2, 1.27.0, 1.27.0 |
| `haven` | 2.5.5 |
| `HDF` | 4.2.15, 4.2.16-2, 4.3.0, 4.3.1 |
| `HDF5` | 1.10.7, 1.12.1, 1.12.2, 1.14.0, 1.14.3, 1.14.5, 1.14.6 |
| `HDF5Array` | 1.38.0 |
| `hdf5r` | 1.3.12 |
| `hdm` | 0.3.2 |
| `HDO.db` | 1.0.0 |
| `hdrcde` | 3.4 |
| `Heap` | 0.80 |
| `HeapDict` | 1.0.1 |
| `heatmap3` | 1.1.9 |
| `heatmaply` | 1.6.0 |
| `help2man` | 1.47.12, 1.47.16, 1.48.3, 1.49.2, 1.49.3 |
| `HepMC3` | 3.2.5, 3.2.6 |
| `here` | 1.0.2 |
| `hexbin` | 1.28.5 |
| `HF-Datasets` | 2.18.0 |
| `HGNChelper` | 0.8.15 |
| `hgu133plus2.db` | 3.13.0 |
| `HH-suite` | 3.3.0 |
| `HiC-Pro` | 3.1.0-R-4.2.1 |
| `HiCBricks` | 1.28.0 |
| `HiCcompare` | 1.32.0 |
| `HiCExplorer` | 3.7.5 |
| `HiCMatrix` | 17, 17.2 |
| `HiddenMarkov` | 1.8-14 |
| `highr` | 0.11 |
| `Highway` | 1.0.3, 1.0.4 |
| `hiredis` | 1.0.2 |
| `HISAT2` | 2.2.1 |
| `hishel` | 0.0.30 |
| `Hmisc` | 5.2-4 |
| `HMMcopy` | 1.52.0 |
| `HMMER` | 3.3.2, 3.4 |
| `hms` | 1.1.4 |
| `Hmsc` | 3.3-7 |
| `Homer` | 2024-07 |
| `Homo.sapiens` | 1.3.1 |
| `Hook::LexWrap` | 0.26 |
| `HPL` | 2.3 |
| `html5lib` | 1.1 |
| `HTML::Entities::Interpolate` | 1.10 |
| `HTML::Form` | 6.12 |
| `HTML::Parser` | 3.83 |
| `HTML::Tagset` | 3.24 |
| `HTML::Template` | 2.97 |
| `HTML::Tree` | 5.07 |
| `htmlTable` | 2.4.3 |
| `htmltools` | 0.5.8.1 |
| `htmlwidgets` | 1.6.4 |
| `HTSlib` | 1.12, 1.14, 1.15.1, 1.17, 1.18, 1.19.1, 1.21, 1.22.1 |
| `HTTP::CookieJar` | 0.014 |
| `HTTP::Cookies` | 6.11 |
| `HTTP::Daemon` | 6.16 |
| `HTTP::Date` | 6.06 |
| `HTTP::Message` | 7.00 |
| `HTTP::Negotiate` | 6.01 |
| `HTTP::Tiny` | 0.090 |
| `httpcode` | 0.3.0 |
| `httpcore` | 1.0.5, 1.0.6 |
| `httpuv` | 1.6.16 |
| `httpx` | 0.27.2 |
| `httr` | 1.4.7 |
| `httr2` | 1.2.1 |
| `huge` | 1.3.5 |
| `huggingface-hub` | 0.21.4, 0.24.6 |
| `humanfriendly` | 10.0 |
| `hunspell` | 3.0.6 |
| `hwloc` | 2.10.0, 2.11.2, 2.12.1, 2.2.0, 2.4.1, 2.5.0, 2.7.1, 2.8.0, 2.9.1, 2.9.2 |
| `hwriter` | 1.3.2.1 |
| `hypergeo` | 1.2-14 |
| `Hyperopt` | 0.2.7 |
| `hypothesis` | 4.57.1-Python-2.7.18, 5.41.2, 6.103.1, 6.13.1, 6.133.2, 6.136.6, 6.14.6, 6.46.7, 6.68.2, 6.82.0, 6.90.0 |
| `Hypre` | 2.20.0, 2.25.0, 2.27.0 |

## I

| Modulename | Versions |
| :--- | :--- |
| `ica` | 1.0-3 |
| `iced` | 0.5.10 |
| `ICU` | 67.1, 69.1, 71.1, 72.1, 73.2, 74.1, 75.1, 76.1, 77.1 |
| `identify` | 2.5.35, 2.6.0 |
| `idl` | 8.9, 9.1 |
| `idna` | 3.10, 3.6 |
| `IDPmisc` | 1.1.21 |
| `idr` | 1.3 |
| `ids` | 1.0.1 |
| `ie2misc` | 0.9.2 |
| `if` | 0.0608 |
| `igraph` | 2.2.1 |
| `IGV` | 2.17.4-Java-17, 2.18.4-Java-17 |
| `igv-reports` | 1.5.1-Python-3.10.4 |
| `IHW` | 1.38.0 |
| `iimkl` | 2025b |
| `iimpi` | 2021a, 2021b, 2022a, 2022b, 2023a, 2025b |
| `IJulia` | 1.24.0-Julia-1.8.5 |
| `IlluminaHumanMethylation450kanno.ilmn12.hg19` | 0.6.1 |
| `IlluminaHumanMethylation450kmanifest` | 0.4.0 |
| `IlluminaHumanMethylationEPICanno.ilm10b2.hg19` | 0.6.0 |
| `IlluminaHumanMethylationEPICanno.ilm10b4.hg19` | 0.6.0 |
| `IlluminaHumanMethylationEPICmanifest` | 0.3.0 |
| `IlluminaHumanMethylationEPICv2anno.20a1.hg38` | 1.0.1 |
| `IlluminaHumanMethylationEPICv2manifest` | 1.0.1 |
| `illuminaio` | 0.52.0 |
| `Ima::DBI` | 0.35 |
| `image.binarization` | 0.1.3 |
| `imagecodecs` | 2022.9.26 |
| `imageio` | 2.13.5, 2.22.2, 2.31.1, 2.33.1 |
| `ImageMagick` | 7.1.0-37, 7.1.0-4, 7.1.1-15, 7.1.1-38, 7.1.1-47, 7.1.2-7 |
| `imager` | 1.0.5 |
| `imagerExtra` | 1.3.2 |
| `imagesize` | 1.4.1 |
| `ImageSmith` | 1.0 |
| `Imath` | 3.1.11, 3.1.12, 3.1.5, 3.1.6, 3.1.7, 3.1.9 |
| `IMB` | 2021.3 |
| `imbalanced-learn` | 0.10.1, 0.12.3 |
| `imgaug` | 0.4.0-CUDA-11.7.0 |
| `imkl` | 2020.4.304, 2021.2.0, 2021.4.0, 2022.1.0, 2022.2.1, 2023.1.0, 2023.2.0, 2025.2.0 |
| `imkl-FFTW` | 2021.4.0, 2022.1.0, 2022.2.1, 2023.1.0, 2025.2.0 |
| `immutabledict` | 4.1.0, 4.2.0, 4.2.1 |
| `immutables` | 0.20, 0.21 |
| `impi` | 2021.16.1, 2021.2.0, 2021.4.0, 2021.6.0, 2021.7.1, 2021.9.0 |
| `Import::Into` | 1.002005 |
| `Importer` | 0.026 |
| `importlib-metadata` | 8.7.0 |
| `importlib-resources` | 6.5.2 |
| `importlib_metadata` | 7.0.1 |
| `impute` | 1.84.0 |
| `imread` | 0.7.4 |
| `ineq` | 0.2-13 |
| `inflection` | 1.3.7 |
| `influenceR` | 0.1.5 |
| `infotheo` | 1.2.0.1 |
| `ini` | 0.3.1 |
| `iniconfig` | 2.0.0, 2.1.0 |
| `Inline` | 0.86, 0.87 |
| `inline` | 0.3.21 |
| `insight` | 1.4.2 |
| `installer` | 0.7.0 |
| `intel` | 2021a, 2021b, 2022a, 2022b, 2023a, 2025b |
| `intel-compilers` | 2021.2.0, 2021.4.0, 2022.1.0, 2022.1.0-GCC-11.2.0, 2022.2.1, 2023.1.0, 2025.2.0 |
| `InteractionSet` | 1.38.0 |
| `interactiveDisplayBase` | 1.48.0 |
| `intergraph` | 2.0-4 |
| `interp` | 1.1-6 |
| `interpretR` | 0.2.5 |
| `intervals` | 0.15.5 |
| `intervaltree` | 0.1, 3.1.0 |
| `intltool` | 0.51.0 |
| `intreehooks` | 1.0 |
| `intrinsicDimension` | 1.2.0 |
| `inum` | 1.0-5 |
| `IO::Compress::Zip` | 2.213 |
| `IO::HTML` | 1.004 |
| `IO::Socket::SSL` | 2.089, 2.095 |
| `IO::String` | 1.08 |
| `IO::Stringy` | 2.113 |
| `IO::TieCombine` | 1.005 |
| `IO::Tty` | 1.20 |
| `ipaddress` | 1.0.23 |
| `IPC::Cmd` | 1.04 |
| `IPC::Run` | 20231003.0 |
| `IPC::Run3` | 0.049 |
| `IPC::System::Simple` | 1.30 |
| `ipred` | 0.9-15 |
| `ipykernel` | 6.29.5 |
| `IPython` | 7.18.1, 7.26.0, 8.12.0, 8.14.0, 8.17.2, 8.28.0, 8.5.0 |
| `ipython` | 8.28.0 |
| `ipython_genutils` | 0.2.0 |
| `ipywidgets` | 8.1.5 |
| `IQ-TREE` | 2.2.2.3, 2.2.2.6, 2.2.2.7, 2.3.5 |
| `irace` | 4.3 |
| `IRanges` | 2.44.0 |
| `irlba` | 2.3.5.1 |
| `ISA-L` | 2.30.0, 2.31.0 |
| `ISL` | 0.26 |
| `ismev` | 1.43 |
| `Iso` | 0.0-21 |
| `isoband` | 0.2.7 |
| `ISOcodes` | 2025.05.18 |
| `IsoformSwitchAnalyzeR` | 1.18.0-R-4.2.1 |
| `ISOweek` | 0.6-2 |
| `isva` | 1.9 |
| `iterators` | 1.0.14 |
| `itertools` | 0.1-3 |
| `ITK` | 5.2.1, 5.3.0 |
| `itksnap` | itksnap-4.2.0 |
| `itsdangerous` | 2.1.2, 2.2.0 |

## J

| Modulename | Versions |
| :--- | :--- |
| `JADE` | 2.0-4 |
| `JAGS` | 4.3.0 |
| `janeaustenr` | 1.0.0 |
| `jaraco.classes` | 3.3.0, 3.4.0 |
| `jaraco.context` | 6.0.1 |
| `jaraco.functools` | 4.1.0, 4.2.1 |
| `JASPAR2020` | 0.99.10 |
| `JasPer` | 2.0.24, 2.0.28, 2.0.33, 4.0.0, 4.2.4, 4.2.5, 4.2.8 |
| `Java` | 1.8.0_192, 11.0.16, 11.0.20, 17.0.6, 21.0.8 |
| `jax` | 0.3.25-CUDA-11.7.0, 0.4.25, 0.4.25-CUDA-12.1.1 |
| `jbigkit` | 2.1 |
| `JBTools` | 0.7.2.9 |
| `jdcal` | 1.4.1 |
| `jedi` | 0.19.1, 0.19.1 |
| `jeepney` | 0.8.0, 0.9.0 |
| `Jellyfish` | 2.3.0 |
| `jemalloc` | 5.2.1, 5.3.0 |
| `jiebaR` | 0.11.1 |
| `jiebaRD` | 0.1 |
| `Jinja2` | 3.1.6 |
| `jmespath` | 1.0.1 |
| `jmp` | 0.0.4 |
| `joblib` | 1.3.2, 1.4.2, 1.5.1 |
| `jomo` | 2.7-6 |
| `jose` | 1.2.1 |
| `jpeg` | 0.1-11 |
| `jq` | 1.6 |
| `jquerylib` | 0.1.4 |
| `JSON` | 4.10 |
| `json-c` | 0.16, 0.17, 0.18 |
| `json-fortran` | 9.0.2 |
| `json5` | 0.9.25 |
| `JSON::MaybeXS` | 1.004008 |
| `JSON::XS` | 4.03 |
| `JsonCpp` | 1.9.4, 1.9.5 |
| `jsonify` | 1.2.2 |
| `jsonlite` | 2.0.0 |
| `jsonschema` | 4.21.0, 4.23.0, 4.25.0 |
| `jsonschema-specifications` | 2025.4.1 |
| `jsonschema_specifications` | 2023.12.1, 2024.10.1 |
| `jstable` | 1.3.19 |
| `Judy` | 1.0.5 |
| `juicyjuice` | 0.1.0 |
| `Julia` | 1.10.4-linux-x86_64, 1.8.5-linux-x86_64, 1.9.0-linux-x86_64, 1.9.2-linux-x86_64, 1.9.3-linux-x86_64 |
| `JUM` | 3.0.0 |
| `jupyter` | 1.1.1 |
| `Jupyter-bundle` | 20230823, 20240522 |
| `jupyter-core` | 5.7.1, 5.7.2 |
| `jupyter-lsp` | 2.2.5 |
| `jupyter-resource-usage` | 1.0.0, 1.0.2 |
| `jupyter-server` | 1.21.0, 2.14.0, 2.14.2, 2.7.0, 2.7.2 |
| `jupyter-server-proxy` | 4.0.0, 4.1.2 |
| `jupyter-telemetry` | 0.1.0 |
| `jupyter_client` | 8.6.3 |
| `jupyter_console` | 6.6.3 |
| `jupyter_core` | 5.7.2 |
| `jupyter_events` | 0.10.0 |
| `jupyter_packaging` | 0.12.3 |
| `jupyter_resource_usage` | 1.0.0 |
| `jupyter_server` | 2.14.2 |
| `jupyter_server_proxy` | 4.0.0 |
| `jupyter_server_terminals` | 0.5.3 |
| `JupyterHub` | 4.0.2, 4.1.5 |
| `jupyterhub` | 4.0.2 |
| `jupyterhub-jwtauthenticator-v2` | 2.0.3 |
| `jupyterhub-ldapauthenticator` | 1.3.2 |
| `jupyterhub-nativeauthenticator` | 1.2.0 |
| `jupyterhub-simplespawner` | 0.1 |
| `jupyterhub-systemdspawner` | 1.0.1 |
| `JupyterLab` | 2.2.8, 3.5.0, 3.5.3, 4.0.3, 4.0.5, 4.2.0, 4.2.5 |
| `jupyterlab` | 4.2.5 |
| `jupyterlab_pygments` | 0.3.0 |
| `jupyterlab_server` | 2.27.3 |
| `jupyterlab_widgets` | 3.0.13 |
| `jupyterlmod` | 4.0.3, 4.0.3 |
| `JupyterNotebook` | 7.0.2, 7.2.0 |
| `jxrlib` | 1.1, 2019.10.9 |

## K

| Modulename | Versions |
| :--- | :--- |
| `kableExtra` | 1.4.0 |
| `KaHIP` | 3.14, 3.16, 3.19 |
| `Kaleido` | 0.2.1, 0.2.1 |
| `Kalign` | 3.3.5, 3.4.0 |
| `kallisto` | 0.48.0 |
| `kde1d` | 1.1.1 |
| `kedd` | 1.0.4 |
| `KEGGgraph` | 1.70.0 |
| `KEGGREST` | 1.50.0 |
| `keras` | 2.13.1, 3.9.2 |
| `kernlab` | 0.9-33 |
| `KernSmooth` | 2.23-26 |
| `keyring` | 24.3.0, 25.6.0 |
| `keyrings.alt` | 5.0.2 |
| `kim-api` | 2.2.1, 2.3.0 |
| `kineto` | 0.4.0 |
| `king` | 2.2.7 |
| `kinship2` | 1.9.6.2 |
| `kiwisolver` | 1.4.5, 1.4.8 |
| `klaR` | 1.7-3 |
| `KMC` | 3.2.4 |
| `knitr` | 1.50 |
| `KODAMA` | 3.0 |
| `kohonen` | 3.0.12 |
| `krbalancing` | 0.5.0b0 |
| `ks` | 1.15.1 |

## L

| Modulename | Versions |
| :--- | :--- |
| `labdsv` | 2.1-2 |
| `labeling` | 0.4.3 |
| `labelled` | 2.16.0 |
| `laeken` | 0.5.3 |
| `lambda.r` | 1.2.4 |
| `LAME` | 3.100 |
| `LAMMPS` | 23Jun2022-kokkos, 29Aug2024-kokkos |
| `LaplacesDemon` | 16.1.6 |
| `lars` | 1.3 |
| `lassosum` | 0.4.5 |
| `later` | 1.4.4 |
| `latex2exp` | 0.9.8 |
| `lattice` | 0.22-7 |
| `latticeExtra` | 0.6-31 |
| `lava` | 1.8.2 |
| `lavaan` | 0.6-20 |
| `lazy` | 1.2-18 |
| `lazy_loader` | 0.3 |
| `lazyeval` | 0.2.2 |
| `LCFdata` | 2.0 |
| `lda` | 1.5.2 |
| `ldap3` | 2.9.1 |
| `ldbounds` | 2.0.2 |
| `LDC` | 1.24.0-x86_64, 1.30.0 |
| `LEA` | 3.22.0 |
| `leafem` | 0.2.5 |
| `leafgl` | 0.2.2 |
| `leaflegend` | 1.2.1 |
| `leaflet` | 2.2.3 |
| `leaflet.providers` | 2.0.0 |
| `leafsync` | 0.1.0 |
| `leaps` | 3.2 |
| `LearnBayes` | 2.15.1 |
| `lefser` | 1.20.0 |
| `legacy_api_wrap` | 1.4 |
| `leiden` | 0.4.3.1 |
| `leidenbase` | 0.1.35 |
| `Leptonica` | 1.83.0 |
| `LERC` | 4.0.0 |
| `lhs` | 1.2.0 |
| `liac-arff` | 2.5.0 |
| `libaec` | 1.0.6, 1.1.3, 1.1.4 |
| `libaio` | 0.3.112, 0.3.113 |
| `libaom` | 3.10.0 |
| `libarchive` | 3.4.3, 3.5.1, 3.6.1, 3.6.2, 3.7.2, 3.7.4, 3.7.7, 3.8.1 |
| `libavif` | 0.11.1 |
| `libcerf` | 1.17, 2.1, 2.3, 3.0 |
| `libclc` | 20.1.8 |
| `libcoin` | 1.0-10 |
| `libdap` | 3.20.11 |
| `libde265` | 1.0.11, 1.0.16 |
| `libdeflate` | 1.10, 1.15, 1.18, 1.19, 1.20, 1.24, 1.8 |
| `libdivsufsort` | 2.0.1 |
| `libdrm` | 2.4.102, 2.4.106, 2.4.107, 2.4.110, 2.4.114, 2.4.115, 2.4.117, 2.4.122, 2.4.125 |
| `libepoxy` | 1.5.10, 1.5.4 |
| `libevent` | 2.1.12 |
| `libfabric` | 1.11.0, 1.12.1, 1.13.2, 1.15.1, 1.16.1, 1.18.0, 1.19.0, 1.21.0, 2.0.0, 2.1.0 |
| `libfdf` | 0.5.1 |
| `libffi` | 3.2.1, 3.3, 3.4.2, 3.4.4, 3.4.5, 3.5.1 |
| `libgd` | 2.3.1, 2.3.3 |
| `libgeotiff` | 1.6.0, 1.7.0, 1.7.1, 1.7.3, 1.7.4 |
| `libgit2` | 1.4.3, 1.5.0, 1.7.1, 1.9.1 |
| `libGLU` | 9.0.1, 9.0.2, 9.0.3 |
| `libglvnd` | 1.3.2, 1.3.3, 1.4.0, 1.6.0, 1.7.0 |
| `libGridXC` | 2.0.2 |
| `libheif` | 1.16.2, 1.19.8, 1.20.2 |
| `libiconv` | 1.16, 1.17, 1.18 |
| `libidn2` | 2.3.2, 2.3.7, 2.3.8 |
| `Libint` | 2.7.2-lmax-6-cp2k |
| `libjpeg-turbo` | 2.0.5, 2.0.6, 2.1.3, 2.1.4, 2.1.5.1, 3.0.1, 3.1.0, 3.1.1 |
| `libjxl` | 0.8.1 |
| `LibLZF` | 3.6 |
| `libnsl` | 1.3.0, 2.0.1 |
| `libogg` | 1.3.5, 1.3.6 |
| `libopus` | 1.3.1, 1.4, 1.5.2 |
| `libpciaccess` | 0.16, 0.17, 0.18.1 |
| `libpng` | 1.2.59, 1.6.37, 1.6.38, 1.6.39, 1.6.40, 1.6.43, 1.6.48, 1.6.50 |
| `libpsl` | 0.21.1, 0.21.5 |
| `libPSML` | 2.1.0 |
| `LibRaw` | 0.21.4 |
| `libreadline` | 8.0, 8.1, 8.1.2, 8.2 |
| `librosa` | 0.10.1, 0.10.1 |
| `libsigc++` | 2.10.8 |
| `libsndfile` | 1.0.31, 1.1.0, 1.2.0, 1.2.2 |
| `libsodium` | 1.0.18, 1.0.19, 1.0.20 |
| `libspatialindex` | 1.9.3 |
| `libtasn1` | 4.19.0 |
| `LibTIFF` | 4.1.0, 4.2.0, 4.3.0, 4.4.0, 4.5.0, 4.6.0, 4.7.0 |
| `libtirpc` | 1.3.1, 1.3.2, 1.3.3, 1.3.4, 1.3.5, 1.3.6 |
| `libtommath` | 1.3.0 |
| `libtool` | 2.4.6, 2.4.7, 2.5.4 |
| `libunistring` | 1.0, 1.3 |
| `libunwind` | 1.4.0, 1.5.0, 1.6.2, 1.8.1, 1.8.2 |
| `libvdwxc` | 0.4.0 |
| `libvorbis` | 1.3.7 |
| `libvori` | 220621 |
| `libvpx` | 1.15.2 |
| `libwebp` | 1.1.0, 1.2.4, 1.3.1, 1.3.2, 1.4.0, 1.5.0 |
| `libxc` | 5.2.3, 6.2.2, 7.0.0 |
| `libxml++` | 2.42.1 |
| `libxml2` | 2.10.3, 2.11.4, 2.11.5, 2.12.7, 2.13.4, 2.14.3, 2.9.10, 2.9.13 |
| `libxslt` | 1.1.34, 1.1.37, 1.1.38, 1.1.42, 1.1.43 |
| `libxsmm` | 1.17 |
| `libyaml` | 0.2.1, 0.2.5 |
| `LiCSBAS2` | 1.9.0 |
| `lifecycle` | 1.0.4 |
| `limma` | 3.66.0 |
| `limSolve` | 2.0.1 |
| `linaro_forge` | 23.0.3, 24.1.1 |
| `Lingua::EN::PluralToSingular` | 0.21 |
| `linkcomm` | 1.0-14 |
| `linprog` | 0.9-4 |
| `liquidSVM` | 1.2.4 |
| `List::AllUtils` | 0.19 |
| `List::MoreUtils` | 0.430 |
| `List::MoreUtils::XS` | 0.430 |
| `List::SomeUtils` | 0.59 |
| `List::UtilsBy` | 0.12 |
| `listenv` | 0.10.0 |
| `lit` | 18.1.7, 18.1.7, 18.1.8, 18.1.8 |
| `litedown` | 0.8 |
| `LittleCMS` | 2.12, 2.13.1, 2.14, 2.15, 2.16, 2.17 |
| `LLVM` | 11.0.0, 11.1.0, 12.0.1, 14.0.3, 14.0.6, 14.0.6-llvmlite, 15.0.5, 16.0.6, 18.1.8, 19.1.7, 20.1.7, 20.1.8 |
| `llvmlite` | 0.43.0 |
| `LMDB` | 0.9.24, 0.9.29, 0.9.31 |
| `lme4` | 1.1-37 |
| `LMERConvenienceFunctions` | 3.0 |
| `lmerTest` | 3.1-3, 3.1-3 |
| `lmom` | 3.2 |
| `Lmoments` | 1.3-2 |
| `lmtest` | 0.9-40 |
| `lobstr` | 1.1.2 |
| `local::lib` | 2.000029 |
| `Locale::Maketext::Simple` | 0.21 |
| `locfdr` | 1.1-8 |
| `locfit` | 1.5-9.12 |
| `locket` | 1.0.0 |
| `lockfile` | 0.12.2 |
| `log4r` | 0.4.4 |
| `Log::Dispatch` | 2.71 |
| `Log::Dispatch::Array` | 1.005 |
| `Log::Dispatchouli` | 3.008, 3.011 |
| `Log::Handler` | 0.90 |
| `Log::Log4perl` | 1.57 |
| `Log::Message` | 0.08 |
| `Log::Message::Simple` | 0.10 |
| `Log::Report` | 1.39, 1.40 |
| `Log::Report::Optional` | 1.07 |
| `logcondens` | 2.1.8 |
| `logger` | 0.4.1 |
| `Logger::Simple` | 2.0 |
| `logging` | 0.10-108 |
| `logistf` | 1.26.1 |
| `logmuse` | 0.2.7, 0.2.8 |
| `logspline` | 2.1.22 |
| `longitudinal` | 1.1.13 |
| `longmemo` | 1.1-4 |
| `longphase` | v2.0 |
| `loo` | 2.8.0 |
| `loompy` | 3.0.7, 3.0.7 |
| `lpSolve` | 5.6.23 |
| `lpsolve` | 5.5.2.11 |
| `lpSolveAPI` | 5.5.2.0-17.14 |
| `lpsymphony` | 1.38.0 |
| `lqa` | 1.0-3 |
| `lsa` | 0.73.3 |
| `LSD2` | 2.3, 2.4.1 |
| `lsei` | 1.3-0 |
| `lslx` | 0.6.11 |
| `Lua` | 5.4.3, 5.4.4, 5.4.6, 5.4.8 |
| `lubridate` | 1.9.4 |
| `lumi` | 2.62.0 |
| `lwgeom` | 0.2-14 |
| `LWP::MediaTypes` | 6.04 |
| `LWP::Protocol::https` | 6.14 |
| `LWP::Simple` | 6.77, 6.79 |
| `lxml` | 4.9.1, 4.9.2, 4.9.3, 5.3.0, 6.0.0 |
| `lz4` | 1.10.0, 1.9.2, 1.9.3, 1.9.4, 4.3.2, 4.3.3 |
| `LZO` | 2.10 |

## M

| Modulename | Versions |
| :--- | :--- |
| `M3Drop` | 1.36.0 |
| `M4` | 1.4.18, 1.4.19, 1.4.20 |
| `MACS2` | 2.2.9.1 |
| `maeparser` | 1.3.0, 1.3.1 |
| `MAFFT` | 7.505-with-extensions, 7.520-with-extensions, 7.526-with-extensions |
| `magic` | 1.6-1 |
| `magick` | 2.9.0 |
| `magma` | 2.6.2-CUDA-11.7.0, 2.7.2-CUDA-12.1.1 |
| `magrittr` | 2.0.4 |
| `Mail::Util` | 2.22 |
| `make` | 4.3, 4.4.1 |
| `makedepend` | 1.0.7 |
| `makedepf90` | 2.8.8 |
| `makeinfo` | 6.7-minimal, 7.0.3, 7.1 |
| `Mako` | 1.1.3, 1.1.4, 1.2.0, 1.2.4, 1.3.10, 1.3.10, 1.3.5, 1.3.5 |
| `MALDIquant` | 1.22.3 |
| `manipulateWidget` | 0.11.1 |
| `manta` | 1.6.0 |
| `mapproj` | 1.2.12 |
| `maps` | 3.4.3 |
| `maptiles` | 0.10.0 |
| `margin` | 2.3.1 |
| `MariaDB` | 10.9.3 |
| `Markdown` | 3.4.4, 3.6, 3.7 |
| `markdown` | 2.0 |
| `markdown-it-py` | 3.0.0 |
| `MarkUpSafe` | 3.0.2 |
| `MarkupSafe` | 2.1.5, 3.0.2 |
| `marray` | 1.88.0 |
| `maSigPro` | 1.82.0 |
| `MASS` | 7.3-65 |
| `MassSpecWavelet` | 1.76.0 |
| `Matching` | 4.10-15 |
| `MatchIt` | 4.7.2 |
| `Math::Bezier` | 0.01 |
| `Math::CDF` | 0.1 |
| `Math::Round` | 0.08 |
| `Math::Utils` | 1.14 |
| `Math::VecStat` | 0.08 |
| `Mathematica` | 14.0.0 |
| `mathjaxr` | 1.8-0 |
| `matlab` | 1.0.4.1, 2022a, 2023a, 2023b, 2024b, 2025b |
| `matplotlib` | 2.2.5-Python-2.7.18, 3.10.3, 3.10.3, 3.10.5, 3.10.5, 3.3.3, 3.4.2, 3.4.3, 3.5.2, 3.7.0, 3.7.2, 3.8.2, 3.9.2 |
| `matplotlib-inline` | 0.1.6 |
| `Matrix` | 1.7-4, 1.7-5 |
| `matrixcalc` | 1.0-6 |
| `MatrixGenerics` | 1.22.0 |
| `MatrixModels` | 0.5-4 |
| `matrixStats` | 1.5.0 |
| `maturin` | 1.1.0, 1.3.1, 1.4.0-Rust-1.75.0, 1.5.0-Rust-1.76.0, 1.6.0, 1.8.3, 1.9.1 |
| `Maven` | 3.6.3, 3.9.7 |
| `maxLik` | 1.5-2.1 |
| `maxlike` | 0.1-12 |
| `maxnet` | 0.1.4 |
| `MBA` | 0.1-2 |
| `mboost` | 2.9-11 |
| `MCE::Mutex` | 1.900, 1.901 |
| `mclogit` | 0.9.6 |
| `mclust` | 6.1.2 |
| `mcmc` | 0.9-8 |
| `MCMCpack` | 1.7-1 |
| `mcmcse` | 1.5-1 |
| `mctc-lib` | 0.3.1 |
| `mda` | 0.5-5 |
| `MDI` | 1.4.29 |
| `mdurl` | 0.1.2 |
| `medflex` | 0.6-11 |
| `mediation` | 4.5.1 |
| `MEDIPS` | 1.62.0 |
| `MEGAHIT` | 1.2.9 |
| `MEME` | 5.5.7 |
| `memisc` | 0.99.31.8.3 |
| `memoise` | 2.0.1 |
| `memuse` | 4.2-3 |
| `Mesa` | 20.2.1, 21.1.1, 21.1.7, 22.0.3, 22.2.4, 23.1.4, 23.1.9, 24.1.3, 25.1.3 |
| `MeshLab` | 2023.12 |
| `Meshroom` | 2023.3.0-CUDA-12.1.1 |
| `Meson` | 0.55.3, 0.58.0, 0.58.2, 0.62.1, 0.64.0, 1.1.1, 1.2.3, 1.4.0, 1.6.1, 1.8.2 |
| `meson-python` | 0.11.0, 0.13.2, 0.15.0, 0.16.0, 0.18.0, 0.18.0 |
| `MESS` | 0.6.0 |
| `Meta::Builder` | 0.004 |
| `MetaboCoreUtils` | 1.18.0 |
| `metadat` | 1.4-0 |
| `MetaEuk` | 6 |
| `metafor` | 4.8-0 |
| `metagenomeSeq` | 1.52.0 |
| `metaMA` | 3.1.3 |
| `metap` | 1.12 |
| `MetaPhlAn` | 4.0.6 |
| `metapod` | 1.18.0 |
| `metashape` | 2.0.1, 2.0.2, 2.1.0, 2.3.0 |
| `MetaUtility` | 2.1.2 |
| `methylartist` | 1.2.6, 1.3.0, 1.5.3, 1.5.3 |
| `methylKit` | 1.36.0 |
| `MethylSeekR` | 1.50.0 |
| `methylumi` | 2.56.0 |
| `METIS` | 5.1.0 |
| `mets` | 1.3.8 |
| `Mfuzz` | 2.70.0 |
| `mgatk` | 0.7.0 |
| `mgcv` | 1.9-3 |
| `mgltools` | 1.5.7 |
| `mgsub` | 1.7.3 |
| `mhsmm` | 0.4.21 |
| `mi` | 1.2 |
| `mia` | 1.18.0 |
| `mice` | 3.18.0 |
| `miceadds` | 3.18-36 |
| `microbenchmark` | 1.5.0 |
| `MicrobiomeStat` | 1.2 |
| `MIIVsem` | 0.5.8 |
| `mime` | 0.13 |
| `MIME::Base32` | 1.303 |
| `MIME::Base64` | 3.16 |
| `MIME::Charset` | v1.013.1 |
| `MIME::Lite` | 3.033 |
| `MIME::Types` | 2.26, 2.28 |
| `minerva` | 1.5.10 |
| `minfi` | 1.56.0 |
| `Miniforge3` | 24.1.2-0, 24.11.3-0, 25.3.0-3 |
| `minimap2` | 2.24, 2.26, 2.28, 2.29 |
| `miniprot` | 0.13 |
| `miniUI` | 0.1.2 |
| `minpack.lm` | 1.2-4 |
| `minqa` | 1.2.8 |
| `minty` | 0.0.5 |
| `mirai` | 2.5.3 |
| `mirt` | 1.45.1 |
| `misc3d` | 0.9-1 |
| `miscTools` | 0.6-28 |
| `missForest` | 1.6.1 |
| `missMDA` | 1.20 |
| `missMethyl` | 1.44.0 |
| `mistune` | 3.0.2 |
| `mitml` | 0.4-5 |
| `mitools` | 2.4 |
| `MiXCR` | 4.6.0-Java-17 |
| `Mixin::Linewise::Readers` | 0.111 |
| `mixOmics` | 6.34.0 |
| `mixsqp` | 0.3-54 |
| `mixtools` | 2.0.0.1 |
| `ml-collections` | 0.1.1 |
| `ml_collections` | 0.1.1 |
| `ml_dtypes` | 0.3.2, 0.5.0, 0.5.0 |
| `mlbench` | 2.1-6 |
| `mlegp` | 3.1.9 |
| `MLInterfaces` | 1.90.0 |
| `MLmetrics` | 1.1.3 |
| `mlogit` | 1.1-3 |
| `mlr` | 2.19.3 |
| `mlrMBO` | 1.1.5.1 |
| `mltools` | 0.3.5 |
| `mm-common` | 1.0.4 |
| `Mmg` | 5.6.0, 5.8.0 |
| `MMseqs2` | 17-b804f |
| `mnormt` | 2.1.1 |
| `mock` | 5.2.0 |
| `Mock::Quick` | 1.111 |
| `modeest` | 2.4.0 |
| `ModelMetrics` | 1.2.2.2 |
| `modelr` | 0.1.11 |
| `modeltools` | 0.2-24 |
| `modin` | 0.32.0, 0.32.0 |
| `modkit` | 0.5.0, 0.6.1 |
| `modsem` | 1.0.13 |
| `Module::Build` | 0.4234 |
| `Module::Build::Tiny` | 0.051, 0.052 |
| `Module::Build::XSUtil` | 0.19 |
| `Module::CoreList` | 5.20241120, 5.20250720 |
| `Module::Implementation` | 0.09 |
| `Module::Install` | 1.21 |
| `Module::Load` | 0.36 |
| `Module::Load::Conditional` | 0.74 |
| `Module::Metadata` | 1.000038 |
| `Module::Path` | 0.19 |
| `Module::Pluggable` | 6.2, 6.3 |
| `Module::Runtime` | 0.016, 0.018 |
| `Module::Runtime::Conflicts` | 0.003 |
| `Molden` | 7.1 |
| `momentfit` | 1.0 |
| `moments` | 0.14.1 |
| `MonteCarlo` | 1.0.6 |
| `Moo` | 2.005005 |
| `Moose` | 2.2207, 2.4000 |
| `MooseX::LazyRequire` | 0.11 |
| `MooseX::OneArgNew` | 0.007 |
| `MooseX::Role::Parameterized` | 1.11 |
| `MooseX::SetOnce` | 0.203 |
| `MooseX::Types` | 0.50, 0.51 |
| `MooseX::Types::Perl` | 0.101344 |
| `more-itertools` | 10.2.0, 10.6.0, 10.7.0 |
| `more_itertools` | 10.7.0 |
| `mosaicCore` | 0.9.5 |
| `mosdepth` | 0.3.3 |
| `motif` | 2.3.8 |
| `MotifDb` | 1.52.0 |
| `motifmatchr` | 1.32.0 |
| `motifStack` | 1.54.0 |
| `Mouse` | v2.5.11 |
| `Mozilla::CA` | 20240924, 20250602 |
| `mpath` | 0.4-2.26 |
| `MPC` | 1.2.1, 1.3.1 |
| `MPFR` | 4.1.0, 4.2.0, 4.2.1, 4.2.2 |
| `mpi4py` | 3.1.4, 3.1.5, 4.0.1, 4.1.0, 4.1.0 |
| `MPICH` | 3.4.2 |
| `MplusAutomation` | 1.2 |
| `mpmath` | 1.3.0 |
| `MrBayes` | 3.2.7 |
| `mRMRe` | 2.1.2.2 |
| `MRO::Compat` | 0.15 |
| `MRtrix` | 3.0.3, 3.0.4 |
| `MsCoreUtils` | 1.21.0 |
| `MsExperiment` | 1.12.0 |
| `MsFeatures` | 1.18.0 |
| `msgpack` | 1.0.7, 1.1.0, 1.1.1 |
| `msgspec` | 0.18.6 |
| `msigdb` | 1.18.0 |
| `msigdbr` | 25.1.1 |
| `msm` | 1.8.2 |
| `MSnbase` | 2.36.0 |
| `MSstats` | 4.18.0 |
| `MSstatsConvert` | 1.20.0 |
| `MSstatsLiP` | 1.16.0 |
| `MSstatsPTM` | 2.12.0 |
| `MSstatsTMT` | 2.18.0 |
| `mstate` | 0.3.3 |
| `mstore` | 0.3.0 |
| `multcomp` | 1.4-29 |
| `multcompView` | 0.1-10 |
| `MultiAssayExperiment` | 1.36.0 |
| `multichoose` | 1.0.3 |
| `multicool` | 1.0.1 |
| `MultiDataSet` | 1.38.0 |
| `multidict` | 6.0.4, 6.1.0 |
| `multipol` | 1.0-9 |
| `MultiQC` | 1.14 |
| `multitaper` | 1.0-17 |
| `multtest` | 2.66.0 |
| `MUMPS` | 5.3.5-metis, 5.4.1-metis, 5.5.1-metis, 5.6.1-metis, 5.7.2-metis |
| `munsell` | 0.5.1 |
| `muscat` | 1.24.0 |
| `MUSCLE` | 5.1.0 |
| `mutoss` | 0.1-13 |
| `mvabund` | 4.2.1 |
| `mvnfast` | 0.2.8 |
| `mvtnorm` | 1.3-3 |
| `mzID` | 1.48.0 |
| `mzR` | 2.44.0 |

## N

| Modulename | Versions |
| :--- | :--- |
| `nabor` | 0.5.0 |
| `NADA` | 1.6-1.2 |
| `NAMD` | 2.14, 3.0-mpi, 3.0b6 |
| `namespace::autoclean` | 0.31 |
| `namespace::clean` | 0.27 |
| `namex` | 0.0.8 |
| `naniar` | 1.1.0 |
| `nanocompore` | 1.0.0 |
| `nanoget` | 1.19.3 |
| `nanomath` | 1.4.0, 1.4.0 |
| `nanomax-analysis-utils` | 0.4.4 |
| `nanonext` | 1.7.2 |
| `NanoPlot` | 1.43.0 |
| `nanopolish` | 0.14.0 |
| `NASM` | 2.15.05, 2.16.01, 2.16.03 |
| `natserv` | 1.0.0 |
| `natsort` | 8.4.0 |
| `naturalsort` | 0.1.3 |
| `nbclassic` | 1.0.0 |
| `nbclient` | 0.10.0 |
| `nbconvert` | 7.16.4 |
| `nbformat` | 5.10.4, 5.9.2 |
| `ncbi-vdb` | 3.0.2 |
| `ncbit` | 2013.03.29.1 |
| `NCCL` | 2.12.12-CUDA-11.7.0, 2.18.3-CUDA-12.1.1, 2.22.3-CUDA-12.6.0, 2.26.6-CUDA-12.8.0, 2.8.3-CUDA-11.1.1 |
| `ncdf4` | 1.24 |
| `ncdfFlow` | 2.56.0 |
| `NCmisc` | 1.2.0 |
| `NCO` | 5.1.3, 5.1.9 |
| `ncurses` | 6.1, 6.2, 6.3, 6.4, 6.5 |
| `ncview` | 2.1.8 |
| `nest_asyncio` | 1.6.0 |
| `Net::Domain` | 3.15 |
| `Net::HTTP` | 6.23 |
| `Net::SMTP::SSL` | 1.04 |
| `Net::SNMP` | v6.0.1 |
| `Net::SSLeay` | 1.94 |
| `netaddr` | 1.3.0 |
| `netCDF` | 4.7.4, 4.8.0, 4.8.1, 4.9.0, 4.9.2, 4.9.3 |
| `netCDF-C++4` | 4.3.1 |
| `netCDF-Fortran` | 4.5.3, 4.6.0, 4.6.1 |
| `netcdf4-python` | 1.5.5.1, 1.6.4 |
| `netifaces` | 0.11.0 |
| `nettle` | 3.10.1, 3.10.2, 3.7.3, 3.8, 3.8.1, 3.9.1 |
| `network` | 1.19.0 |
| `networkDynamic` | 0.11.5 |
| `networkLite` | 1.1.0 |
| `networkx` | 2.5, 2.6.3, 2.8.4, 3.0, 3.1, 3.2.1, 3.4.2 |
| `neuralnet` | 1.44.2 |
| `neuRosim` | 0.2-14 |
| `Nextflow` | 23.10.0, 24.04.2, 24.10.0, 24.10.2, 24.10.9, 25.04.7 |
| `nf-core` | 2.13.1, 2.13.1, 2.14.1, 2.14.1 |
| `NFFT` | 3.5.3 |
| `ngspatial` | 1.2-2 |
| `NiBabel` | 3.2.1, 5.3.2 |
| `nibabel` | 5.3.2 |
| `Nilearn` | 0.12.1 |
| `nilearn` | 0.12.1 |
| `Nim` | 1.6.6 |
| `Ninja` | 1.10.1, 1.10.2, 1.11.1, 1.12.1, 1.13.0 |
| `NISTunits` | 1.0.1 |
| `nleqslv` | 3.3.5 |
| `nlme` | 3.1-168 |
| `nlohmann_json` | 3.10.5, 3.11.2, 3.11.3, 3.12.0 |
| `NLopt` | 2.10.0, 2.7.0, 2.7.1 |
| `nloptr` | 2.2.1 |
| `NLP` | 0.3-2 |
| `nlsem` | 0.8-1 |
| `NLTK` | 3.8.1, 3.8.1 |
| `NMF` | 0.28 |
| `nnet` | 7.3-20 |
| `nnls` | 1.6 |
| `nodeenv` | 1.8.0, 1.9.1 |
| `nodejs` | 16.15.1, 18.12.1, 18.17.1, 20.13.1, 20.9.0, 22.16.0, 22.17.1 |
| `NOISeq` | 2.54.0 |
| `nonnest2` | 0.5-8 |
| `nor1mix` | 1.3-3 |
| `norm` | 1.0-11.1 |
| `nortest` | 1.0-4 |
| `notebook` | 7.0.8 |
| `notebook_shim` | 0.2.4 |
| `np` | 0.60-18 |
| `npsurv` | 0.5-0 |
| `NSPR` | 4.29, 4.30, 4.32, 4.34, 4.35, 4.36, 4.37 |
| `NSS` | 3.104, 3.113, 3.114, 3.57, 3.65, 3.69, 3.79, 3.85, 3.89.1, 3.94 |
| `nsync` | 1.25.0, 1.26.0, 1.29.2 |
| `NTSS` | 0.1.3 |
| `numactl` | 2.0.13, 2.0.14, 2.0.16, 2.0.18, 2.0.19 |
| `numba` | 0.56.4, 0.56.4-CUDA-11.7.0, 0.58.0, 0.58.1, 0.60.0, 0.60.0 |
| `numbat` | 1.5.1 |
| `Number::Compare` | 0.03 |
| `Number::Format` | 1.76 |
| `numDeriv` | 2016.8-1.1 |
| `numexpr` | 2.11.0, 2.8.1, 2.8.4 |
| `numpy` | 2.3.1, 2.3.2 |
| `numpy-groupies` | 0.10.2 |
| `numpydoc` | 1.8.0 |
| `NVHPC` | 21.11, 22.11-CUDA-11.7.0, 23.1-CUDA-12.0.0 |

## O

| Modulename | Versions |
| :--- | :--- |
| `oai` | 0.4.0 |
| `oauthlib` | 3.2.2 |
| `Object::Accessor` | 0.48 |
| `Object::InsideOut` | 4.05 |
| `occt` | 7.5.0p1 |
| `oce` | 1.8-3 |
| `OceanView` | 1.0.8 |
| `oddsratio` | 2.0.2 |
| `officer` | 0.7.0 |
| `oligo` | 1.74.0 |
| `oligoClasses` | 1.72.0 |
| `onetimepass` | 1.0.1 |
| `ontologyIndex` | 2.12 |
| `oompaBase` | 3.2.10 |
| `oompaData` | 3.1.5 |
| `OPARI2` | 2.0.6, 2.0.7, 2.0.8, 2.0.9 |
| `openair` | 2.19.0 |
| `OpenBabel` | 3.1.1 |
| `OpenBLAS` | 0.3.12, 0.3.15, 0.3.18, 0.3.20, 0.3.20-int8, 0.3.21, 0.3.23, 0.3.24, 0.3.27, 0.3.29, 0.3.30 |
| `OpenCV` | 4.5.1-contrib, 4.6.0-CUDA-11.7.0-contrib, 4.6.0-contrib |
| `openCyto` | 2.22.0 |
| `OpenEXR` | 2.5.5, 3.1.1, 3.1.5, 3.1.7, 3.2.0, 3.2.4, 3.3.4 |
| `OpenFOAM` | 10, 11, 12, 13, 7-20200508, 9, v2112, v2206, v2306, v2406, v2506, v2512 |
| `OpenGL` | 2025.09 |
| `OpenJPEG` | 2.4.0, 2.5.0, 2.5.2, 2.5.3 |
| `OpenMM` | 7.7.0-foss-2022a-CUDA-11.7.0, 8.0.0, 8.0.0-CUDA-12.1.1, 8.0.0-foss-2022a-CUDA-11.7.0 |
| `OpenMolcas` | 22.10 |
| `OpenMPI` | 4.0.5, 4.1.1, 4.1.4, 4.1.5, 4.1.6, 5.0.3, 5.0.7, 5.0.8 |
| `OpenMx` | 2.22.9 |
| `OpenPGM` | 5.2.122 |
| `OpenPNM` | 3.3.0 |
| `openpyxl` | 3.1.2, 3.1.2, 3.1.5, 3.1.5 |
| `OpenSSL` | 1.1, 3 |
| `openssl` | 2.3.4 |
| `openxlsx` | 4.2.8.1 |
| `operator.tools` | 1.6.3 |
| `opt-einsum` | 3.3.0 |
| `opt_einsum` | 3.3.0, 3.4.0 |
| `opt_einsum_fx` | 0.1.4 |
| `Optax` | 0.2.2-CUDA-12.1.1 |
| `optax` | 0.2.2 |
| `optextras` | 2019-12.4 |
| `optimParallel` | 1.0-2 |
| `optimr` | 2019-12.16 |
| `optimx` | 2025-4.9 |
| `optmatch` | 0.10.8 |
| `optparse` | 1.7.5 |
| `optree` | 0.14.1 |
| `Optuna` | 2.10.0 |
| `orbax_checkpoint` | 0.5.15, 0.5.18 |
| `ORCA` | 5.0.3, 5.0.4, 6.0.0, 6.0.1, 6.0.1-avx2, 6.1.0-avx2 |
| `ordinal` | 2023.12-4.1 |
| `org.Hs.eg.db` | 3.22.0 |
| `org.Mm.eg.db` | 3.22.0 |
| `org.Rn.eg.db` | 3.22.0 |
| `OrganismDbi` | 1.52.0 |
| `origami` | 1.0.7 |
| `oro.nifti` | 0.11.4 |
| `orthopolynom` | 1.0-6.1 |
| `Osi` | 0.108.11, 0.108.8, 0.108.9 |
| `OSPRay` | 2.12.0 |
| `osqp` | 0.6.3.3 |
| `otel` | 0.2.0 |
| `OTF2` | 2.3, 3.0.2, 3.0.3, 3.1.1 |
| `outliers` | 0.15 |
| `OUTRIDER` | 1.28.0 |
| `overrides` | 7.7.0 |
| `OVITO` | 3.11.0-basic |
| `oyaml` | 1.0 |

## P

| Modulename | Versions |
| :--- | :--- |
| `p7zip` | 17.04 |
| `Package::Constants` | 0.06 |
| `Package::DeprecationManager` | 0.18 |
| `Package::Stash` | 0.40 |
| `Package::Stash::XS` | 0.30 |
| `packaging` | 23.1, 23.2, 24.2, 25.0 |
| `packrat` | 0.9.3 |
| `pacman` | 0.5.1 |
| `PadWalker` | 2.5 |
| `pagmo` | 2.19.1 |
| `pairtools` | v1.1.0 |
| `pamela` | 1.1.0 |
| `pammtools` | 0.7.3 |
| `pamr` | 1.57 |
| `pan` | 1.9 |
| `pandas` | 2.3.0, 2.3.1 |
| `pander` | 0.6.6 |
| `Pandoc` | 3.1.2 |
| `pandocfilters` | 1.5.1 |
| `Pango` | 1.47.0, 1.48.5, 1.48.8, 1.50.12, 1.50.14, 1.50.7, 1.51.0, 1.54.0, 1.56.3, 1.57.0 |
| `PAPI` | 7.0.0, 7.0.1, 7.1.0 |
| `parallel` | 20210322, 20220722, 20230722 |
| `Parallel::ForkManager` | 2.03 |
| `parallelDist` | 0.2.7 |
| `parallelly` | 1.45.1 |
| `parallelMap` | 1.5.1 |
| `parameterized` | 0.9.0 |
| `ParamHelpers` | 1.14.2 |
| `Params::Check` | 0.38 |
| `Params::Util` | 1.102 |
| `Params::Validate` | 1.31 |
| `Params::ValidationCompiler` | 0.31 |
| `ParaView` | 5.10.1-mpi, 5.11.1, 5.11.2, 5.13.2, 5.9.1-mpi, 6.0.1 |
| `ParaView-Catalyst` | 2.0.0 |
| `parent` | 0.243, 0.244 |
| `ParMETIS` | 4.0.3 |
| `Parse::RecDescent` | 1.967015 |
| `Parse::Yapp` | 1.21 |
| `parsedate` | 1.3.2 |
| `parso` | 0.8.3 |
| `partd` | 1.4.0, 1.4.2 |
| `party` | 1.3-18 |
| `partykit` | 1.2-24 |
| `pastecs` | 1.4.2 |
| `pastel` | 0.2.1 |
| `patchelf` | 0.17.2, 0.18.0 |
| `patchwork` | 1.3.2 |
| `Path::Tiny` | 0.146, 0.150 |
| `pathlib2` | 2.3.7.post1 |
| `pathspec` | 0.12.1 |
| `pathview` | 1.50.0 |
| `patsy` | 0.5.6 |
| `pbapply` | 1.7-4 |
| `pbivnorm` | 0.6.0 |
| `pbkrtest` | 0.5.5 |
| `pbmcapply` | 1.5.1 |
| `pbr` | 6.1.1 |
| `pbs-installer` | 2024.9.9 |
| `pbs_installer` | 2025.4.9, 2025.7.23 |
| `PBZIP2` | 1.1.13 |
| `PCAmatchR` | 0.3.3 |
| `pcaMethods` | 2.2.0 |
| `pcaPP` | 2.0-5 |
| `PCRE` | 8.44, 8.45 |
| `PCRE2` | 10.35, 10.36, 10.37, 10.40, 10.42, 10.43, 10.45 |
| `PDAL` | 2.8.2 |
| `PDBFixer` | 1.9 |
| `PDF::API2` | 2.047 |
| `pdiff` | 1.1.4 |
| `PDM` | 2.18.2 |
| `pdm` | 2.18.2 |
| `pdm_backend` | 2.3.3 |
| `pdp` | 0.8.2 |
| `PDT` | 3.25.1, 3.25.2 |
| `PEAR` | 0.9.11 |
| `PearsonDS` | 1.3.2 |
| `pec` | 2025.06.24 |
| `penalized` | 0.9-53 |
| `penfa` | 0.1.1 |
| `peperr` | 1.6 |
| `pephubclient` | 0.4.4 |
| `peppy` | 0.40.1, 0.40.6 |
| `performance` | 0.15.2 |
| `Perl` | 5.32.0, 5.32.0-minimal, 5.32.1, 5.32.1-minimal, 5.34.0, 5.34.1, 5.34.1-minimal, 5.36.0, 5.36.0-minimal, 5.36.1, 5.38.0, 5.38.2, 5.40.0, 5.40.2, 5.42.0 |
| `Perl-bundle-CPAN` | 5.36.1, 5.38.0, 5.38.2, 5.40.0, 5.40.2 |
| `Perl::OSType` | 1.010 |
| `Perl::PrereqScanner` | 1.100 |
| `PerlIO::utf8_strict` | 0.010 |
| `perm` | 1.0-0.4 |
| `PermAlgo` | 1.2 |
| `permute` | 0.9-8 |
| `PETSc` | 3.14.4, 3.17.4 |
| `pexpect` | 4.9.0 |
| `PFAM.db` | 3.22.0 |
| `phangorn` | 2.12.1 |
| `pheatmap` | 1.0.13 |
| `phono3py` | 2.7.0 |
| `phonopy` | 2.20.0 |
| `phylobase` | 0.8.12 |
| `PhyloPhlAn` | 3.0.3 |
| `phyloseq` | 1.54.0 |
| `phylowgs` | v1.0 |
| `PhyML` | 3.3.20220408 |
| `phytools` | 2.5-2 |
| `picard` | 2.25.1-Java-11 |
| `pickleshare` | 0.7.5 |
| `pigz` | 2.8 |
| `pillar` | 1.11.1 |
| `Pillow` | 10.0.0, 10.2.0, 10.4.0, 11.3.0, 8.0.1, 8.2.0, 8.3.2, 9.1.1, 9.4.0 |
| `Pillow-SIMD` | 10.4.0, 9.2.0, 9.5.0 |
| `pim` | 2.0.4 |
| `pinfsc50` | 1.3.0 |
| `Pint` | 0.23 |
| `pip` | 24.3.1, 25.1.1 |
| `piper` | 0.14.0, 0.14.2 |
| `pipestat` | 0.10.2, 0.8.2 |
| `pixman` | 0.40.0, 0.42.2, 0.43.4, 0.46.2, 0.46.4 |
| `pixmap` | 0.4-14 |
| `pkg-config` | 0.29.2 |
| `pkgbuild` | 1.4.8 |
| `pkgconf` | 1.8.0, 1.9.3, 1.9.5, 2.0.3, 2.2.0, 2.3.0, 2.4.3 |
| `pkgconfig` | 1.5.1-python, 1.5.4-python, 1.5.5-python, 2.0.3 |
| `pkgdown` | 2.1.3 |
| `pkginfo` | 1.12.1.2, 1.9.6 |
| `pkgload` | 1.4.1 |
| `pkgmaker` | 0.32.10 |
| `PKI` | 0.1-15 |
| `plac` | 1.4.2, 1.4.3 |
| `platformdirs` | 3.11.0, 4.3.6, 4.3.7, 4.3.8 |
| `PLINK` | 1.9b_6.21-x86_64, 2.00a2.3, 2.00a3.7 |
| `plinkliftover` | 0.3.0, 0.3.0 |
| `plogr` | 0.2.0 |
| `plot3D` | 1.4.2 |
| `plot3Drgl` | 1.0.5 |
| `plotly` | 4.11.0, 5.16.0 |
| `plotly.py` | 5.12.0, 5.16.0, 5.4.0 |
| `plotmo` | 3.6.4 |
| `plotrix` | 3.8-4 |
| `pls` | 2.8-5 |
| `pluggy` | 1.2.0, 1.5.0, 1.6.0 |
| `PLUMED` | 2.7.2, 2.7.3, 2.8.0, 2.9.0, 2.9.2 |
| `PLY` | 3.11 |
| `ply` | 3.11 |
| `plyr` | 1.8.9 |
| `plyranges` | 1.30.0 |
| `PMA` | 1.2-4 |
| `PMIx` | 3.1.5, 3.2.3, 4.1.0, 4.1.2, 4.2.2, 4.2.4, 4.2.6, 5.0.2, 5.0.6, 5.0.8 |
| `pmp` | 1.21.0 |
| `PnetCDF` | 1.12.2, 1.12.3, 1.13.0 |
| `png` | 0.1-8 |
| `pocl` | 1.8 |
| `Pod::Elemental` | 0.103006 |
| `Pod::Escapes` | 1.07 |
| `Pod::Eventual` | 0.094003 |
| `Pod::LaTeX` | 0.61 |
| `Pod::Man` | v6.0.2 |
| `Pod::Parser` | 1.67 |
| `Pod::Plainer` | 1.04 |
| `Pod::POM` | 2.01 |
| `Pod::Simple` | 3.45, 3.47 |
| `Pod::Weaver` | 4.020 |
| `poetry` | 1.5.1, 1.6.1, 1.7.1, 1.7.1, 1.8.3, 2.1.2, 2.1.2, 2.1.3, 2.1.3 |
| `poetry-core` | 2.1.2, 2.1.3 |
| `poetry-plugin-export` | 1.9.0 |
| `poetry_core` | 1.8.1 |
| `poetry_plugin_export` | 1.6.0 |
| `PoissonSeq` | 1.1.2 |
| `polars` | 0.20.2 |
| `poLCA` | 1.6.0.1 |
| `polspline` | 1.1.25 |
| `Polychrome` | 1.5.4 |
| `polyclip` | 1.10-7 |
| `polycor` | 0.8-1 |
| `polynom` | 1.4-1 |
| `pomegranate` | 0.15.0 |
| `pooch` | 1.8.2 |
| `poppler` | 22.01.0, 22.12.0, 23.09.0 |
| `portpicker` | 1.5.2 |
| `posterior` | 1.6.1 |
| `PostgreSQL` | 16.1, 16.4, 17.5 |
| `POV-Ray` | 3.7.0.10 |
| `poweRlaw` | 1.0.0 |
| `ppcor` | 1.1 |
| `PPI` | 1.279, 1.283 |
| `prabclus` | 2.3-4 |
| `pracma` | 2.4.6 |
| `praise` | 1.0.0 |
| `pre-commit` | 3.7.0, 3.8.0 |
| `predicts` | 0.1-19 |
| `preprocessCore` | 1.72.0 |
| `PresenceAbsence` | 1.1.11 |
| `preseqR` | 4.0.0 |
| `prettyGraphs` | 2.2.0 |
| `prettyunits` | 1.2.0 |
| `princurve` | 2.1.6 |
| `pROC` | 1.19.0.1 |
| `processx` | 3.8.6 |
| `prodigal` | 2.6.3 |
| `prodlim` | 2025.04.28 |
| `profileModel` | 0.6.1 |
| `proftools` | 0.99-3 |
| `profvis` | 0.4.0 |
| `progress` | 1.2.3 |
| `progressr` | 0.17.0 |
| `PROJ` | 7.2.1, 8.0.1, 8.1.0, 9.0.0, 9.1.1, 9.2.0, 9.3.1, 9.4.1, 9.6.2 |
| `projpred` | 2.9.1 |
| `pRoloc` | 1.50.0 |
| `pRolocdata` | 1.48.0 |
| `pRolocGUI` | 2.20.0 |
| `prometheus_client` | 0.17.1, 0.21.0 |
| `promises` | 1.3.3, 1.5.0 |
| `prompt-toolkit` | 3.0.36 |
| `prompt_toolkit` | 3.0.41 |
| `propcache` | 0.2.0 |
| `ProtGenerics` | 1.42.0 |
| `proto` | 1.0.0 |
| `protobuf` | 23.0, 24.0, 25.3, 28.0, 3.14.0, 3.19.4 |
| `protobuf-python` | 3.19.4, 4.23.0, 4.24.0, 4.25.3, 5.28.0 |
| `proxy` | 0.4-27 |
| `proxyC` | 0.5.2 |
| `PRROC` | 1.4 |
| `PRRTE` | 3.0.11, 3.0.5, 3.0.8 |
| `PRSice` | 2.3.5, v2.3.5 |
| `pryr` | 0.1.6 |
| `ps` | 1.9.1 |
| `PSCBS` | 0.68.0 |
| `pscl` | 1.5.9 |
| `PSMatch` | 1.14.0 |
| `pspline` | 1.0-21 |
| `psutil` | 6.0.0, 6.0.0, 7.0.0, 7.0.0 |
| `psych` | 2.5.6 |
| `psycopg` | 3.1.18 |
| `psycopg2` | 2.9.9 |
| `ptyprocess` | 0.7.0 |
| `Publish` | 2025.07.24 |
| `pugixml` | 1.12.1, 1.14 |
| `PuLP` | 2.7.0, 2.8.0, 2.8.0, 3.1.1 |
| `pulsar` | 0.3.11 |
| `pure_eval` | 0.2.2 |
| `PureCN` | 2.16.0 |
| `purrr` | 1.1.0, 1.2.0 |
| `pvclust` | 2.2-0 |
| `pwalign` | 1.6.0 |
| `PWEALL` | 1.3.0.1 |
| `py` | 1.11.0 |
| `py-cpuinfo` | 9.0.0 |
| `py-expression-eval` | 0.3.14 |
| `py3Dmol` | 2.1.0, 2.1.0 |
| `PyAMG` | 5.0.1 |
| `pyarrow` | 17.0.0, 22.0.0 |
| `pyasn1` | 0.6.1 |
| `pyasn1-modules` | 0.3.0 |
| `pyasn1_modules` | 0.4.1 |
| `PyAV` | 11.0.0 |
| `pybedtools` | 0.9.0, 0.9.1 |
| `PyBIDS` | 0.21.0 |
| `pyBigWig` | 0.3.18, 0.3.22 |
| `pybind11` | 2.10.3, 2.11.1, 2.12.0, 2.13.6, 2.6.0, 2.6.2, 2.7.1, 2.7.1-Python-2.7.18, 2.9.2, 3.0.0 |
| `PyCairo` | 1.21.0 |
| `PyCharm` | 2024.1.6 |
| `pyclibrary` | 0.2.2 |
| `pycparser` | 2.22 |
| `pycryptodome` | 3.22.0, 3.23.0 |
| `PycURL` | 7.45.2, 7.45.3 |
| `pydantic` | 2.5.3, 2.5.3, 2.6.4, 2.6.4, 2.9.1, 2.9.1 |
| `pydantic_core` | 2.14.6, 2.16.3, 2.23.3 |
| `pydevtool` | 0.3.0 |
| `pydicom` | 2.2.2, 3.0.1 |
| `PyEVTK` | 2.0.0 |
| `pyevtk` | 1.6.0 |
| `pyFAI` | 0.21.3 |
| `pyfaidx` | 0.7.1, 0.7.2.1, 0.8.1.1, 0.8.1.2 |
| `pyGenomeTracks` | 3.8, 3.9 |
| `pygimli` | pg1.5.4 |
| `Pygments` | 2.19.1, 2.19.2 |
| `pygmo` | 2.19.5 |
| `pygmtsar` | 2024.4.17-virt |
| `PyGObject` | 3.42.1 |
| `PyJWT` | 2.8.0 |
| `pylev` | 1.4.0 |
| `Pylint` | 3.2.5 |
| `pyls` | pyls |
| `PyMOL` | 2.5.0 |
| `PyNaCl` | 1.5.0 |
| `pynndescent` | 0.5.11 |
| `pynvml` | 11.4.1 |
| `PyOpenCL` | 2021.2.13 |
| `PyOpenGL` | 3.1.6, 3.1.7 |
| `pyOpenSSL` | 23.2.0 |
| `pypardiso` | 0.4.0, 0.4.2 |
| `pyparsing` | 3.2.3 |
| `pyproj` | 3.6.0 |
| `pyproject-hooks` | 1.1.0 |
| `pyproject-metadata` | 0.7.1, 0.8.0, 0.9.1 |
| `pyproject_hooks` | 1.0.0, 1.1.0, 1.2.0 |
| `pyproject_metadata` | 0.8.0 |
| `PyQt-builder` | 1.15.4, 1.16.4, 1.18.1 |
| `PyQt5` | 5.15.10, 5.15.11, 5.15.4, 5.15.5, 5.15.7 |
| `pyqt_builder` | 1.18.1 |
| `PyQtGraph` | 0.13.3 |
| `PyRosetta` | 4.release-387, 4.release-387 |
| `pyrsistent` | 0.20.0 |
| `Pysam` | 0.17.0, 0.18.0, 0.19.1, 0.21.0, 0.22.0, 0.22.1, 0.23.3 |
| `PyTables` | 3.8.0 |
| `pytest` | 7.4.2, 7.4.2, 8.3.5, 8.4.1 |
| `pytest-flakefinder` | 1.1.0 |
| `pytest-mock` | 3.11.1, 3.14.0, 3.14.1 |
| `pytest-rerunfailures` | 12.0, 14.0, 15.0 |
| `pytest-shard` | 0.1.2 |
| `pytest-subtests` | 0.13.1 |
| `pytest-workflow` | 2.1.0, 2.1.0 |
| `pytest-xdist` | 2.5.0, 3.3.1, 3.3.1, 3.6.1, 3.8.0 |
| `Python` | 2.7.18, 2.7.18-bare, 3.10.4, 3.10.4-bare, 3.10.8, 3.10.8-bare, 3.11.3, 3.11.5, 3.12.3, 3.13.1, 3.13.5, 3.8.6, 3.9.5, 3.9.5-bare, 3.9.6, 3.9.6-bare |
| `Python-bundle` | 3.10.4, 3.10.8 |
| `Python-bundle-PyPI` | 2023.06, 2023.10, 2024.06, 2025.04, 2025.07 |
| `python-crfsuite` | 0.9.10 |
| `python-dateutil` | 2.9.0.post0 |
| `Python-Deprecated` | 1.1.0 |
| `python-dotenv` | 1.0.1 |
| `python-isal` | 1.1.0, 1.6.1, 1.7.0 |
| `python-json-logger` | 2.0.7 |
| `python-lsp-server` | 1.12.0 |
| `python-lzo` | 1.15 |
| `python-slugify` | 8.0.4 |
| `python-xxhash` | 3.4.1 |
| `python-zlib-ng` | 0.5.1 |
| `pythran` | 0.18.0 |
| `pytoml` | 0.1.21 |
| `PyTorch` | 1.12.0-CUDA-11.7.0, 1.12.1-CUDA-11.7.0, 2.1.2, 2.1.2-foss-2023a-CUDA-12.1.1, 2.6.0 |
| `pytz` | 2025.2 |
| `PyVista` | 0.43.8 |
| `PyWavelets` | 1.1.1, 1.4.1 |
| `PyYAML` | 5.3.1, 5.4.1, 6.0, 6.0.1, 6.0.2 |
| `PyZMQ` | 24.0.1, 25.1.1, 25.1.2, 26.2.0 |

## Q

| Modulename | Versions |
| :--- | :--- |
| `qap` | 0.1-2 |
| `qctool` | v2.2.0rel |
| `QDNAseq` | 1.46.0 |
| `QFeatures` | 1.20.0 |
| `qgam` | 2.0.0 |
| `QGIS` | 3.30.3, 4.0.1 |
| `qgraph` | 1.9.8 |
| `Qhull` | 2020.2 |
| `qlcMatrix` | 0.9.9 |
| `qqconf` | 1.3.2 |
| `qqman` | 0.1.9 |
| `QRank` | QRank_1.0 |
| `qrng` | 0.0-10 |
| `qrnn` | 2.1.1 |
| `Qt5` | 5.15.10, 5.15.13, 5.15.16, 5.15.2, 5.15.5, 5.15.7 |
| `Qt6` | 6.6.3, 6.9.3 |
| `Qtconsole` | 5.3.2, 5.6.0 |
| `qtcreator` | 10.0.1, 14.0.1, 15.0.0 |
| `QtPy` | 2.2.1, 2.3.0, 2.4.1 |
| `quadprog` | 1.5-8, 1.5.8-R4.2.1 |
| `Qualimap` | 2.3-R-4.4.1 |
| `quanteda` | 4.3.1 |
| `quantmod` | 0.4.28 |
| `quantreg` | 6.1 |
| `quantsmooth` | 1.76.0 |
| `QuantumESPRESSO` | 7.1, 7.3.1, 7.4, 7.4.1 |
| `questionary` | 2.0.1 |
| `questionr` | 0.8.1 |
| `QuickJSR` | 1.8.1 |
| `qvalue` | 2.42.0 |
| `Qwt` | 6.2.0 |

## R

| Modulename | Versions |
| :--- | :--- |
| `R` | 4.2.1, 4.3.2, 4.4.1, 4.5.1, 4.5.2 |
| `R-bundle-Bioconductor` | 3.15-R-4.2.1, 3.18-R-4.3.2, 3.18-R-4.4.1, 3.19-R-4.4.1, 3.22-R-4.5.2 |
| `R-bundle-CRAN` | 2023.12, 2023.12-R-4.4.1, 2025.11 |
| `R.cache` | 0.17.0 |
| `R.devices` | 2.17.2 |
| `R.filesets` | 2.15.1 |
| `R.huge` | 0.10.1 |
| `R.matlab` | 3.7.0 |
| `R.methodsS3` | 1.8.2 |
| `R.oo` | 1.27.1 |
| `R.rsp` | 0.46.0 |
| `R.utils` | 2.13.0 |
| `r2rtf` | 1.2.0 |
| `R2WinBUGS` | 2.1-23 |
| `R6` | 2.6.1 |
| `ragg` | 1.5.0 |
| `rainbow` | 3.8 |
| `RAMSES` | ramses-20240314 |
| `random` | 0.2.6 |
| `randomcoloR` | 1.1.0.1 |
| `randomForest` | 4.7-1.2 |
| `randomForestSRC` | 3.4.3 |
| `randtoolbox` | 2.0.5 |
| `rangeModelMetadata` | 0.1.5 |
| `ranger` | 0.17.0 |
| `RANN` | 2.6.2 |
| `rapidfuzz` | 3.13.0, 3.6.1 |
| `RapidJSON` | 1.1.0, 1.1.0-20230928, 1.1.0-20240815, 1.1.0-20250205 |
| `rapidjsonr` | 1.2.0 |
| `RAPIDS` | rapids-25.06 |
| `rappdirs` | 0.3.3 |
| `rARPACK` | 0.11-0 |
| `raster` | 3.6-32 |
| `rasterio` | 1.4.3, 1.4.3 |
| `rasterVis` | 0.51.7 |
| `ratelimitr` | 0.4.2 |
| `Ray` | 2.37.0, 2.9.1 |
| `Ray-project` | 2.37.0, 2.9.1 |
| `RBesT` | 1.8-2 |
| `RBGL` | 1.86.0 |
| `rbibutils` | 2.3 |
| `rbiom` | 2.2.1 |
| `rbison` | 1.0.0 |
| `Rborist` | 0.3-11 |
| `RCAL` | 2.0 |
| `Rcgmin` | 2022-4.30 |
| `RCircos` | 1.2.2 |
| `RcisTarget` | 1.29.0 |
| `rclone` | 1.63.1-amd64 |
| `rcmdcheck` | 1.4.0 |
| `RColorBrewer` | 1.1-3 |
| `Rcpp` | 1.1.0 |
| `RcppAnnoy` | 0.0.22 |
| `RcppArmadillo` | 15.0.2-2 |
| `RcppEigen` | 0.3.4.0.2 |
| `RcppGSL` | 0.3.13 |
| `RcppHNSW` | 0.6.0 |
| `RcppML` | 0.3.7 |
| `RcppParallel` | 5.1.11-1 |
| `RcppProgress` | 0.4.2 |
| `RcppRoll` | 0.3.1 |
| `RcppThread` | 2.2.0 |
| `RcppTOML` | 0.2.3 |
| `RcppZiggurat` | 0.1.8 |
| `RCurl` | 1.98-1.17 |
| `rda` | 1.2-1 |
| `Rdpack` | 2.6.4 |
| `rdrop2` | 0.8.2.1 |
| `RE2` | 2022-02-01, 2022-06-01, 2023-03-01, 2023-08-01, 2024-07-02, 2025-07-22 |
| `re2c` | 2.0.3, 2.1.1, 2.2, 3.0, 3.1, 4.2, 4.3 |
| `reactable` | 0.4.4 |
| `reactome.db` | 1.95.0 |
| `ReactomePA` | 1.54.0 |
| `reactR` | 0.6.1 |
| `readbitmap` | 0.1.5 |
| `reader` | 1.0.6 |
| `readODS` | 2.3.2 |
| `Readonly` | 2.05 |
| `readr` | 2.1.5 |
| `readxl` | 1.4.5 |
| `rebird` | 1.3.0 |
| `recipes` | 1.3.1 |
| `Redis` | 6.2.6, 7.2.3, 7.4.1 |
| `redis-py` | 5.0.1, 5.0.1, 5.1.1, 5.1.1 |
| `Ref::Util` | 0.204 |
| `referencing` | 0.32.1, 0.35.1, 0.36.2 |
| `RefFreeEWAS` | 2.2 |
| `refgenconf` | 0.12.2 |
| `refgenie` | 0.12.1 |
| `reformulas` | 0.4.2 |
| `Regenie` | 3.1.2 |
| `regex` | 2023.12.25, 2024.11.6, 2024.7.24, 2025.7.31 |
| `Regexp::Common` | 2024080801 |
| `regioneR` | 1.42.0 |
| `registry` | 0.5-1 |
| `regsem` | 1.9.5 |
| `reldist` | 1.7-2 |
| `relsurv` | 2.3-3 |
| `remaCor` | 0.0.20 |
| `rematch` | 2.0.0 |
| `rematch2` | 2.1.2 |
| `REMORA` | 2.0 |
| `remotes` | 2.5.0 |
| `rentrez` | 1.2.4 |
| `renv` | 1.1.5 |
| `RepeatMasker` | 4.2.1 |
| `ReportingTools` | 2.50.0 |
| `reportlab` | 3.6.12, 4.0.0 |
| `reprex` | 2.1.1 |
| `requests` | 2.31.0, 2.32.3, 2.32.4 |
| `requests-cache` | 1.2.0, 1.2.1 |
| `requests-oauthlib` | 1.3.1 |
| `requests-toolbelt` | 1.0.0 |
| `reretry` | 0.11.8 |
| `resample` | 0.6 |
| `resampy` | 0.4.3 |
| `reshape` | 0.8.10 |
| `reshape2` | 1.4.4 |
| `ResidualMatrix` | 1.20.0 |
| `resolvelib` | 1.0.1 |
| `restfulr` | 0.0.16 |
| `reticulate` | 1.44.0 |
| `rex` | 1.2.1 |
| `Rfast` | 2.1.5.2 |
| `rfc3339_validator` | 0.1.4 |
| `rfc3986_validator` | 0.1.1 |
| `RFdiffusion` | 1.1.0 |
| `RFOC` | 3.4-10 |
| `rgbif` | 3.8.3 |
| `RGCCA` | 3.0.3 |
| `rgexf` | 0.16.3 |
| `rgl` | 1.3.24 |
| `Rglpk` | 0.6-5.1 |
| `Rgraphviz` | 2.54.0 |
| `rhandsontable` | 0.3.8 |
| `rhdf5` | 2.54.0 |
| `rhdf5filters` | 1.22.0 |
| `Rhdf5lib` | 1.32.0 |
| `RhpcBLASctl` | 0.23-42 |
| `Rhtslib` | 3.6.0 |
| `rich` | 10.16.2, 13.2.0, 14.0.0, 14.1.0 |
| `rich-click` | 1.8.8, 1.8.9 |
| `ridge` | 3.3 |
| `ridigbio` | 0.4.1 |
| `RInside` | 0.2.19 |
| `rio` | 1.2.4 |
| `riskRegression` | 2025.09.17 |
| `ritis` | 1.0.0 |
| `RItools` | 0.3-5 |
| `rJava` | 1.0-11 |
| `rjson` | 0.2.23 |
| `RJSONIO` | 2.0.0 |
| `rlang` | 1.1.6 |
| `rle` | 0.10.0 |
| `rlecuyer` | 0.3-8 |
| `rlemon` | 0.2.1 |
| `rlist` | 0.4.6.2 |
| `rmarkdown` | 2.30 |
| `RMBlast` | 2.14.1 |
| `rmeta` | 3.0 |
| `Rmpfr` | 1.1-2 |
| `rms` | 8.1-0 |
| `RMTstat` | 0.3.1 |
| `rmutil` | 1.1.10 |
| `Rnanoflann` | 0.0.3 |
| `RNASeqPower` | 1.50.0 |
| `RnBeads` | 2.28.0 |
| `RnBeads.hg19` | 1.42.0 |
| `RnBeads.hg38` | 1.42.1 |
| `RnBeads.mm10` | 2.18.0 |
| `RnBeads.mm9` | 1.42.0 |
| `RnBeads.rn5` | 1.42.0 |
| `rncl` | 0.8.7 |
| `rnetcarto` | 0.2.6 |
| `RNeXML` | 2.4.11 |
| `rngtools` | 1.5.2 |
| `rngWELL` | 0.10-10 |
| `RNifti` | 1.8.0 |
| `robustbase` | 0.99-6 |
| `ROC` | 1.86.0 |
| `ROCR` | 1.0-11 |
| `ROCR-Runtime` | 4.5.0 |
| `ROCT-Thunk-Interface` | 4.5.0 |
| `ROI` | 1.0-1 |
| `ROI.plugin.glpk` | 1.0-0 |
| `Role::HasMessage` | 0.007 |
| `Role::Identifiable::HasIdent` | 0.009 |
| `Role::Tiny` | 2.002004 |
| `roman-numerals-py` | 3.1.0 |
| `ROntoTools` | 2.38.0 |
| `Rook` | 1.2 |
| `ROOT` | 6.24.06, 6.26.06, 6.30.06 |
| `rootSolve` | 1.8.2.4 |
| `ropls` | 1.42.0 |
| `roptim` | 0.1.7 |
| `rotl` | 3.1.0 |
| `roxygen2` | 7.3.3 |
| `rpact` | 4.2.1 |
| `rpart` | 4.1.24 |
| `rpds-py` | 0.24.0, 0.26.0 |
| `rpds_py` | 0.17.1, 0.20.0 |
| `rpf` | 1.0.15 |
| `RPMG` | 2.2-7 |
| `RPMM` | 1.25 |
| `rpmrebuild` | 2.11 |
| `RPostgreSQL` | 0.7-8 |
| `rprojroot` | 2.1.1 |
| `RProtoBufLib` | 2.22.0 |
| `RPushbullet` | 0.3.5 |
| `rrcov` | 1.7-7 |
| `rredlist` | 1.1.1 |
| `rsa` | 4.9 |
| `rsample` | 1.3.1 |
| `Rsamtools` | 2.26.0 |
| `rsconnect` | 1.6.1 |
| `RSEIS` | 4.2-4 |
| `RSEM` | 1.3.3 |
| `Rserve` | 1.8-15 |
| `RSNNS` | 0.4-17 |
| `Rsolnp` | 2.0.1 |
| `RSpectra` | 0.16-2 |
| `RSQLite` | 2.4.3 |
| `Rssa` | 1.1 |
| `rstan` | 2.32.7 |
| `rstantools` | 2.5.0 |
| `rstatix` | 0.7.3 |
| `rstudio` | 2024.04.0-735, 2024.04.2-764 |
| `rstudioapi` | 0.17.1 |
| `Rsubread` | 2.24.0 |
| `rsvd` | 1.0.5 |
| `rtdists` | 0.11-5 |
| `rtracklayer` | 1.70.0 |
| `Rtree` | 1.2.0 |
| `Rtsne` | 0.17 |
| `Rttf2pt1` | 1.3.14 |
| `ruamel.yaml` | 0.17.21, 0.17.32, 0.17.32 |
| `ruamel.yaml.base` | 0.3.2 |
| `ruamel.yaml.clib` | 0.2.7 |
| `ruamel.yaml.cmd` | 0.6.5 |
| `ruamel.yaml.convert` | 0.3.2 |
| `RUnit` | 0.4.33.1 |
| `Rust` | 1.52.1, 1.54.0, 1.56.0, 1.60.0, 1.65.0, 1.70.0, 1.73.0, 1.75.0, 1.76.0, 1.78.0, 1.83.0, 1.85.1, 1.86.0, 1.88.0, 1.91.1 |
| `ruv` | 0.9.7.1 |
| `rversions` | 2.1.2, 3.0.0 |
| `rvertnet` | 0.8.4 |
| `rvest` | 1.0.5 |
| `rvinecopulib` | 0.7.3.1.0 |
| `Rvmmin` | 2018-4.17.1 |
| `Rwave` | 2.6-5 |
| `RWeka` | 0.4-46 |
| `RWekajars` | 3.9.3-2 |

## S

| Modulename | Versions |
| :--- | :--- |
| `s2` | 1.1.9 |
| `s3fs` | 2024.9.0, 2024.9.0 |
| `s3transfer` | 0.10.3 |
| `S4Arrays` | 1.10.0 |
| `S4Vectors` | 0.48.0 |
| `S7` | 0.2.0 |
| `Safe::Isa` | 1.000010 |
| `safestringlib` | 20240228 |
| `Safetensors` | 0.4.3, 0.4.4 |
| `safetensors` | 0.4.3, 0.4.4 |
| `Salmon` | 1.9.0 |
| `Sambamba` | 1.0.1 |
| `sample_pkg` | version |
| `sampling` | 2.11 |
| `samr` | 3.0 |
| `SamSPECTRAL` | 1.64.0 |
| `SAMtools` | 1.14, 1.16.1, 1.17, 1.18, 1.19.2, 1.21 |
| `sandwich` | 3.1-1 |
| `sass` | 0.4.10 |
| `SBdecomp` | 1.2 |
| `SC3` | 1.38.0 |
| `ScaFaCoS` | 1.0.1, 1.0.4 |
| `ScaLAPACK` | 2.1.0, 2.1.0-fb, 2.2.0-fb, 2.2.2-fb |
| `Scalar::Util` | 1.68, 1.70 |
| `Scalar::Util::Numeric` | 0.40 |
| `Scalasca` | 2.6.1, 2.6.2 |
| `ScaledMatrix` | 1.18.0 |
| `Scalene` | 1.5.26, 1.5.26 |
| `scales` | 1.4.0 |
| `scam` | 1.2-20 |
| `scandir` | 1.10.0 |
| `scanpy` | 1.9.8, 1.9.8 |
| `SCANVIS` | 1.24.0 |
| `scater` | 1.38.0 |
| `scattermore` | 1.2 |
| `scatterpie` | 0.2.6 |
| `scatterplot3d` | 0.3-44 |
| `scDblFinder` | 1.24.0 |
| `scikit-bio` | 0.5.7 |
| `scikit-build` | 0.11.1, 0.15.0, 0.17.6, 0.18.1, 0.18.1 |
| `scikit-build-core` | 0.10.6, 0.11.1, 0.11.5, 0.5.0, 0.9.3 |
| `scikit-image` | 0.18.1, 0.19.1, 0.19.3, 0.21.0, 0.22.0, 0.22.0 |
| `scikit-learn` | 1.0.1, 1.0.2, 1.1.2, 1.2.1, 1.3.1, 1.4.0, 1.4.2, 1.5.2, 1.5.2, 1.7.1, 1.7.1 |
| `scikit-misc` | 0.1.4 |
| `scikit_build_core` | 0.10.6, 0.11.1, 0.11.5, 0.5.0 |
| `scipy` | 1.16.0, 1.16.1 |
| `SciPy-bundle` | 2020.11, 2020.11-Python-2.7.18, 2021.05, 2021.10, 2021.10-Python-2.7.18, 2022.05, 2023.02, 2023.07, 2023.11, 2024.05, 2025.06, 2025.07 |
| `scistreer` | 1.2.0 |
| `SCons` | 4.1.0.post1, 4.2.0, 4.4.0 |
| `Scope::Guard` | 0.21 |
| `Score-P` | 8.0, 8.1, 8.4, 9.0 |
| `SCOTCH` | 6.1.0, 6.1.2, 7.0.1, 7.0.10, 7.0.3, 7.0.6, 7.0.8 |
| `scran` | 1.38.0 |
| `scrime` | 1.3.5 |
| `scs` | 3.2.7 |
| `sctransform` | 0.4.2, 0.4.3 |
| `scuttle` | 1.20.0 |
| `SDL2` | 2.0.20, 2.0.22, 2.26.3, 2.28.2, 2.28.5, 2.30.6, 2.32.10, 2.32.8 |
| `sdsl-lite` | 2.0.3 |
| `Seaborn` | 0.11.2, 0.12.1, 0.12.2, 0.13.2 |
| `SEACR` | 1.3 |
| `SecretStorage` | 3.3.3 |
| `seewave` | 2.2.4 |
| `segmented` | 2.1-4 |
| `selectr` | 0.4-2 |
| `sem` | 3.1-16 |
| `semantic-version` | 2.10.0 |
| `semPLS` | 1.0-10 |
| `semTools` | 0.5-7 |
| `Send2Trash` | 1.8.3 |
| `sendmailR` | 1.4-0 |
| `sensemakr` | 0.1.6 |
| `sentometrics` | 1.0.1 |
| `SEPP` | 4.5.1, 4.5.5 |
| `SeqArray` | 1.50.0 |
| `Seqinfo` | 1.0.0 |
| `seqinr` | 4.2-36 |
| `SeqKit` | 2.8.2 |
| `SeqLib` | 1.2.0 |
| `seqLogo` | 1.76.0 |
| `seqPattern` | 1.42.0 |
| `SeqVarTools` | 1.48.0 |
| `Serf` | 1.3.9 |
| `seriation` | 1.5.8 |
| `servr` | 0.32 |
| `session-info` | 1.0.0 |
| `sessioninfo` | 1.2.3 |
| `Set::Array` | 0.30 |
| `Set::IntervalTree` | 0.12 |
| `Set::IntSpan` | 1.19 |
| `Set::IntSpan::Fast` | 1.15 |
| `Set::Object` | 1.42 |
| `Set::Scalar` | 1.29 |
| `setRNG` | 2024.2-1, 2024.2-1 |
| `setuptools` | 64.0.3, 75.6.0, 80.9.0 |
| `setuptools-rust` | 1.11.0, 1.11.0, 1.6.0, 1.8.0, 1.9.0 |
| `setuptools_scm` | 8.1.0, 8.3.1 |
| `Seurat` | 4.2.0-R-4.2.1, 5.0.2, 5.1.0-R-4.3.2, 5.1.0-R-4.4.1, 5.3.1, 5.4.0, 5.4.0-R-4.5.2 |
| `SeuratObject` | 5.2.0, 5.4.0 |
| `sf` | 1.0-21, 1.1-1 |
| `sfheaders` | 0.4.4 |
| `sfsmisc` | 1.1-22 |
| `sftime` | 0.3.1 |
| `sgdGMF` | 1.0.1 |
| `shadowtext` | 0.1.6 |
| `shape` | 1.4.6.1 |
| `shapefiles` | 0.7.2 |
| `SHAPEIT4` | 4.2.2 |
| `Shapely` | 1.8.2, 2.0.1 |
| `Shell` | 0.73 |
| `shellingham` | 1.5.4 |
| `shiny` | 1.11.1 |
| `shinyBS` | 0.61.1 |
| `shinycssloaders` | 1.1.0 |
| `shinydashboard` | 0.7.3 |
| `shinydashboardPlus` | 2.0.6 |
| `shinyFiles` | 0.9.3 |
| `shinyhelper` | 0.3.2 |
| `shinyjs` | 2.1.0 |
| `shinypanel` | 0.1.5 |
| `shinystan` | 2.6.0 |
| `shinythemes` | 1.2.0 |
| `shinyWidgets` | 0.9.0 |
| `ShortRead` | 1.68.0 |
| `Siesta` | 5.2.2 |
| `siggenes` | 1.84.0 |
| `Signac` | 1.16.0 |
| `signal` | 1.8-1 |
| `SignifReg` | 4.3 |
| `silx` | 1.0.0 |
| `SimBu` | 1.12.0 |
| `SimDesign` | 2.21 |
| `simex` | 1.8 |
| `simona` | 1.8.0 |
| `simpervisor` | 1.0.0 |
| `Simple-DFTD3` | 1.2.1 |
| `simplegeneric` | 0.8.1 |
| `SimpleITK` | 2.1.1.2 |
| `simplejson` | 3.20.1 |
| `simplifyEnrichment` | 2.4.0 |
| `SimSeq` | 1.4.0 |
| `SimToExp` | 2.0 |
| `SingleCellExperiment` | 1.32.0 |
| `SingleR` | 2.12.0 |
| `SIONlib` | 1.7.6-tools, 1.7.7-tools |
| `SIP` | 6.10.0, 6.8.1, 6.8.3 |
| `sitmo` | 2.0.2 |
| `six` | 1.16.0, 1.17.0 |
| `SKAT` | 2.2.5 |
| `sklearn` | 0.0 |
| `slam` | 0.1-55 |
| `SLEPc` | 3.17.2 |
| `slider` | 0.3.2 |
| `slingshot` | 2.18.0 |
| `slippymath` | 0.3.1 |
| `sm` | 2.2-6.0 |
| `smart-open` | 6.4.0, 7.1.0 |
| `smithwaterman` | 20160702 |
| `smmap` | 5.0.0, 5.0.1 |
| `smoof` | 1.6.0.3 |
| `smoother` | 1.3 |
| `smoppix` | 1.2.1 |
| `SMVar` | 1.3.4 |
| `sn` | 2.1.1 |
| `sna` | 2.8 |
| `snakemake` | 7.32.3, 8.27.0, 8.27.0, 8.4.2, 8.4.2 |
| `snakemake-executor-plugin-cluster-generic` | 1.0.7, 1.0.9 |
| `snakemake-executor-plugin-cluster-sync` | 0.1.3, 0.1.4 |
| `snakemake-executor-plugin-flux` | 0.1.0, 0.1.1 |
| `snakemake-executor-plugin-slurm` | 0.12.0, 0.2.1 |
| `snakemake-executor-plugin-slurm-jobstep` | 0.1.10, 0.2.1 |
| `snakemake-interface-common` | 1.15.2, 1.17.4 |
| `snakemake-interface-executor-plugins` | 8.2.0, 9.3.3 |
| `snakemake-interface-report-plugins` | 1.1.0 |
| `snakemake-interface-storage-plugins` | 3.0.0, 3.3.0 |
| `snappy` | 1.1.10, 1.1.8, 1.1.9, 1.2.1, 1.2.2 |
| `SNFtool` | 2.3.1 |
| `sniffio` | 1.3.1 |
| `Sniffles` | 2.4, 2.4, 2.7.1, 2.7.1 |
| `snow` | 0.4-4 |
| `SnowballC` | 0.7.1 |
| `snowballstemmer` | 2.2.0, 3.0.1 |
| `snowfall` | 1.84-6.3 |
| `snowflakeauth` | 0.2.0 |
| `snpEff` | 5.0e-Java-11 |
| `SNPRelate` | 1.44.0 |
| `snpStats` | 1.60.0 |
| `SOAR` | 0.99-11 |
| `SOCI` | 4.0.3, 4.0.3-Boost-1.83.0 |
| `Socket` | 2.038, 2.040 |
| `Software::License` | 0.104006, 0.104007 |
| `SoftwareTree` | Milan |
| `solrium` | 1.2.0 |
| `som` | 0.3-5.2 |
| `sortedcontainers` | 2.4.0 |
| `soundecology` | 1.3.3 |
| `soundfile` | 0.12.1 |
| `soupsieve` | 2.5 |
| `sourcetools` | 0.1.7-1 |
| `soxr` | 0.3.7 |
| `sp` | 2.2-0, 2.2-1 |
| `spaa` | 0.2.5 |
| `spacefillr` | 0.4.0 |
| `SpaceRanger` | 2.1.0 |
| `spacesXYZ` | 1.6-0 |
| `spacetime` | 1.3-3 |
| `spacexr` | 1.2.0-R-4.5.2 |
| `Spack` | 0.21.2 |
| `SPAdes` | 3.15.4, 4.1.0 |
| `SPAM` | 0.6.3.2 |
| `spam` | 2.11-1 |
| `spaMM` | 4.6.1 |
| `Spark` | 3.3.1 |
| `SparseArray` | 1.10.1 |
| `sparsehash` | 2.0.4 |
| `SparseM` | 1.84-2 |
| `sparseMatrixStats` | 1.22.0 |
| `sparsesvd` | 0.2-3 |
| `sparsevctrs` | 0.3.4 |
| `SPAtest` | 3.1.2 |
| `spatial` | 7.3-18 |
| `SpatialExperiment` | 1.20.0 |
| `SpatialPack` | 0.4-1 |
| `spatialreg` | 1.4-2 |
| `spatstat` | 3.4-1 |
| `spatstat.data` | 3.1-9 |
| `spatstat.explore` | 3.5-3, 3.8-0 |
| `spatstat.geom` | 3.6-0, 3.7-3 |
| `spatstat.linnet` | 3.3-2 |
| `spatstat.model` | 3.4-2 |
| `spatstat.random` | 3.4-2, 3.4-5 |
| `spatstat.sparse` | 3.1-0 |
| `spatstat.univar` | 3.1-4, 3.1-7 |
| `spatstat.utils` | 3.2-0, 3.2-2 |
| `spData` | 2.3.4 |
| `spdep` | 1.4-1 |
| `spdlog` | 1.11.0, 1.12.0 |
| `Specio` | 0.48, 0.51 |
| `SpectrA` | 1.0.1 |
| `Spectra` | 1.20.0 |
| `spglib` | 2.1.0, 2.5.0 |
| `spglib-python` | 2.0.0, 2.1.0, 2.5.0 |
| `Sphinx` | 8.2.3 |
| `sphinx-bootstrap-theme` | 0.8.1 |
| `sphinxcontrib-applehelp` | 2.0.0 |
| `sphinxcontrib-devhelp` | 2.0.0 |
| `sphinxcontrib-htmlhelp` | 2.1.0 |
| `sphinxcontrib-jsmath` | 1.0.1 |
| `sphinxcontrib-qthelp` | 2.0.0 |
| `sphinxcontrib-serializinghtml` | 2.0.0 |
| `sphinxcontrib-websupport` | 2.0.0 |
| `SPIA` | 2.62.0 |
| `Spiffy` | 0.46 |
| `spin` | 0.14 |
| `splancs` | 2.01-45 |
| `splitstackshape` | 1.4.8 |
| `spls` | 2.3-2 |
| `SPM` | 25.01.02 |
| `spocc` | 1.2.3 |
| `SPOTlight` | 1.14.0 |
| `spring` | v1.1.1 |
| `spThin` | 0.2.0 |
| `SQL::Abstract` | 2.000001 |
| `SQL::Statement` | 1.414 |
| `SQLAlchemy` | 2.0.25, 2.0.29 |
| `SQLite` | 3.33.0, 3.35.4, 3.36, 3.38.3, 3.39.4, 3.42.0, 3.43.1, 3.45.3, 3.47.2, 3.50.1 |
| `SQUAREM` | 2021.1 |
| `SRA-Toolkit` | 3.0.3 |
| `SSW` | 1.1 |
| `stable` | 1.1.6 |
| `stabledist` | 0.7-2 |
| `stabs` | 0.6-4 |
| `Stack` | 2.13.1-x86_64 |
| `stack_data` | 0.6.3 |
| `stageR` | 1.32.0 |
| `StanHeaders` | 2.32.10 |
| `STAR` | 2.7.10b, 2.7.11a, 2.7.8a, 2.7.9a |
| `STAR-CCM+` | 16.02.009-r8, 17.02.007-r8, 18.06.006-r8, 19.06.009-r8, 20.06.010-double |
| `STAR-Fusion` | v1.13.0 |
| `stargazer` | 5.2.3 |
| `stars` | 0.6-8 |
| `startupmsg` | 1.0.0 |
| `statip` | 0.2.3 |
| `Statistics::Basic` | 1.6611 |
| `Statistics::Descriptive` | 3.0801 |
| `StatMatch` | 1.4.3 |
| `statmod` | 1.5.1 |
| `statnet` | 2019.6 |
| `statnet.common` | 4.12.0 |
| `statsmodels` | 0.13.1, 0.14.0, 0.14.1, 0.14.1 |
| `stdlib_list` | 0.10.0 |
| `stdReg` | 3.4.2 |
| `stopit` | 1.1.2 |
| `stopwords` | 2.3 |
| `Storable` | 3.25 |
| `strictures` | 2.000006 |
| `String::Errf` | 0.009 |
| `String::Flogger` | 1.101246 |
| `String::Formatter` | 1.235 |
| `String::Print` | 0.94 |
| `String::RewritePrefix` | 0.009 |
| `String::Truncate` | 1.100603 |
| `String::TtyLength` | 0.03 |
| `stringdist` | 0.9.15 |
| `stringi` | 1.8.7 |
| `stringmagic` | 1.2.0 |
| `stringr` | 1.5.2, 1.6.0 |
| `strobealign` | v0.14.0 |
| `strucchange` | 1.5-4 |
| `styler` | 1.11.0 |
| `Sub::Exporter` | 0.991 |
| `Sub::Exporter::ForMethods` | 0.100055 |
| `Sub::Exporter::GlobExporter` | 0.006 |
| `Sub::Exporter::Progressive` | 0.001013 |
| `Sub::Identify` | 0.14 |
| `Sub::Info` | 0.002 |
| `Sub::Install` | 0.929 |
| `Sub::Name` | 0.27, 0.28 |
| `Sub::Quote` | 2.006008 |
| `Sub::Uplevel` | 0.2800 |
| `subplex` | 1.9 |
| `Subread` | 2.0.4 |
| `subread` | 1.6.3-virt |
| `subset-bam` | 1.1.0 |
| `Subversion` | 1.14.1 |
| `SuiteSparse` | 5.13.0-METIS-5.1.0, 5.8.1-METIS-5.1.0, 7.1.0, 7.11.0 |
| `SummarizedExperiment` | 1.40.0 |
| `SuperLearner` | 2.0-29 |
| `SuperLU` | 5.3.0 |
| `SuperLU_DIST` | 8.1.0 |
| `SuppDists` | 1.1-9.9 |
| `survey` | 4.4-8 |
| `survival` | 3.8-3 |
| `survivalROC` | 1.0.3.1 |
| `susieR` | 0.14.2 |
| `sva` | 3.58.0 |
| `svd` | 0.5.8 |
| `SVG` | 2.87 |
| `svglite` | 2.2.2 |
| `svMisc` | 1.4.3 |
| `SVT-AV1` | 3.1.2 |
| `svUnit` | 1.0.8 |
| `swagger` | 5.17.14.1 |
| `swan` | 41.42_17Oct2023, gitlab_17Jan2024_openmp, gitlab_24May2024_openmp |
| `SWIG` | 4.0.2, 4.1.1, 4.2.1, 4.3.1 |
| `Switch` | 2.17 |
| `symmoments` | 1.2.1 |
| `sympy` | 1.10.1, 1.11.1, 1.12, 1.13.3 |
| `sys` | 3.4.3 |
| `Sys::Info` | 0.7811 |
| `Sys::Info::Base` | 0.7807 |
| `Sys::Info::Driver::Linux` | 0.7905 |
| `Sys::Info::Driver::Unknown` | 0.79 |
| `systemfonts` | 1.3.1 |
| `Szip` | 2.1.1 |

## T

| Modulename | Versions |
| :--- | :--- |
| `tabixpp` | 1.1.0, 1.1.2 |
| `tableone` | 0.13.2 |
| `tabletools` | 0.1.0 |
| `tabulate` | 0.9.0 |
| `TailRank` | 3.2.4 |
| `Task::Weaken` | 1.06 |
| `tau` | 0.0-26 |
| `taxize` | 0.10.0 |
| `tbb` | 2020.3, 2021.10.0, 2021.11.0, 2021.13.0, 2021.5.0 |
| `tblib` | 2.0.0, 3.0.0 |
| `Tcl` | 8.6.10, 8.6.11, 8.6.12, 8.6.13, 8.6.14, 8.6.16, 9.0.1 |
| `tcltk2` | 1.6.1 |
| `tclust` | 2.1-2 |
| `tcsh` | 6.22.03, 6.24.01, 6.24.10, 6.24.13 |
| `TeachingDemos` | 2.13 |
| `TELEMAC-MASCARET` | 8p3r1, 8p5r0, 9.0.0 |
| `Template` | 3.102 |
| `Template::Plugin::Number::Format` | 1.06 |
| `tenacity` | 8.2.3 |
| `tensor` | 1.5.1 |
| `tensorA` | 0.36.2.1 |
| `tensorboard` | 2.13.0, 2.15.1, 2.18.0, 2.18.0 |
| `tensorboard-data-server` | 0.7.1 |
| `tensorboard-plugin-profile` | 2.13.1 |
| `tensorboard-plugin-wit` | 1.8.1 |
| `tensorboard_data_server` | 0.7.2 |
| `TensorFlow` | 2.11.0-CUDA-11.7.0, 2.13.0, 2.13.0, 2.15.1-foss-2023a-CUDA-12.1.1, 2.18.1, 2.18.1, 2.18.1-CUDA-12.6.0, 2.9.1-CUDA-11.7.0 |
| `tensorflow-estimator` | 2.13.0 |
| `tensorstore` | 0.1.60, 0.1.65, 0.1.65 |
| `tergm` | 4.2.2 |
| `Term::Encoding` | 0.03 |
| `Term::ReadKey` | 2.38 |
| `Term::ReadLine::Gnu` | 1.46, 1.47 |
| `Term::Table` | 0.023, 0.024 |
| `Term::UI` | 0.50 |
| `termcolor` | 2.3.0, 2.5.0 |
| `terminado` | 0.18.1 |
| `terra` | 1.8-80 |
| `tesseract` | 5.3.0 |
| `Test` | 1.26 |
| `test-drive` | 0.5.0 |
| `Test2::Plugin::NoWarnings` | 0.10 |
| `Test2::Require::Module` | 1.302204, 1.302214 |
| `Test::Base` | 0.89 |
| `Test::CheckDeps` | 0.010 |
| `Test::ClassAPI` | 1.07 |
| `Test::CleanNamespaces` | 0.24 |
| `Test::Deep` | 1.204, 1.205 |
| `Test::Differences` | 0.71, 0.72 |
| `Test::Exception` | 0.43 |
| `Test::FailWarnings` | 0.008 |
| `Test::Fatal` | 0.017 |
| `Test::File` | 1.993, 1.995 |
| `Test::File::ShareDir::Dist` | 1.001002 |
| `Test::Harness` | 3.50, 3.52 |
| `Test::LeakTrace` | 0.17 |
| `Test::Memory::Cycle` | 1.06 |
| `Test::More::UTF8` | 0.05 |
| `Test::Most` | 0.38 |
| `Test::Needs` | 0.002010 |
| `Test::NoWarnings` | 1.06 |
| `Test::Object` | 0.08 |
| `Test::Output` | 1.034, 1.036 |
| `Test::Pod` | 1.52 |
| `Test::Requires` | 0.11 |
| `Test::RequiresInternet` | 0.05 |
| `Test::Simple` | 1.302204, 1.302214 |
| `Test::SubCalls` | 1.10 |
| `Test::Sys::Info` | 0.23 |
| `Test::Version` | 2.09 |
| `Test::Warn` | 0.37 |
| `Test::Warnings` | 0.033, 0.038 |
| `Test::Without::Module` | 0.23 |
| `Test::YAML` | 1.07 |
| `testit` | 0.13 |
| `testthat` | 3.2.3 |
| `TEtranscripts` | 2.2.0, 2.2.3 |
| `texlive` | 20220321, 20230313 |
| `texreg` | 1.39.4 |
| `text-unidecode` | 1.3 |
| `Text::Aligner` | 0.16 |
| `Text::Balanced` | 2.06 |
| `Text::CSV` | 2.04, 2.06 |
| `Text::CSV_XS` | 1.57, 1.61 |
| `Text::Diff` | 1.45 |
| `Text::Format` | 0.63 |
| `Text::Glob` | 0.11 |
| `Text::Iconv` | 1.7 |
| `Text::ParseWords` | 3.31 |
| `Text::Soundex` | 3.05 |
| `Text::Table` | 1.135 |
| `Text::Table::Manifold` | 1.03 |
| `Text::Template` | 1.61 |
| `textcat` | 1.0-9 |
| `textplot` | 0.2.2 |
| `textshaping` | 1.0.3, 1.0.4 |
| `textual` | 0.54.0, 0.79.1 |
| `TFBSTools` | 1.48.0 |
| `TFisher` | 0.2.0 |
| `TFMPvalue` | 0.0.9 |
| `TH.data` | 1.1-4 |
| `threadpoolctl` | 3.6.0 |
| `threejs` | 0.3.4 |
| `throttler` | 1.2.2 |
| `Throwable` | 1.001 |
| `tibble` | 3.3.0 |
| `tictoc` | 1.2.1 |
| `tidybayes` | 3.0.7 |
| `tidygraph` | 1.3.1 |
| `tidyr` | 1.3.1 |
| `tidyselect` | 1.2.1 |
| `tidytext` | 0.4.3 |
| `tidytree` | 0.4.6 |
| `tidyverse` | 2.0.0 |
| `Tie::Function` | 0.02 |
| `Tie::IxHash` | 1.23 |
| `tiff` | 0.1-12 |
| `tifffile` | 2023.7.18 |
| `tiktoken` | 0.7.0 |
| `Time::Local` | 1.35 |
| `Time::Piece` | 1.3401, 1.36 |
| `Time::Piece::MySQL` | 0.06 |
| `timechange` | 0.3.0 |
| `timeDate` | 4051.111 |
| `timereg` | 2.0.7 |
| `timeSeries` | 4041.111 |
| `tinycss2` | 1.3.0 |
| `tinytex` | 0.57 |
| `Tk` | 8.6.10, 8.6.11, 8.6.12, 8.6.13, 8.6.14, 8.6.16, 9.0.1 |
| `Tkinter` | 2.7.18, 3.10.4, 3.10.8, 3.11.3, 3.11.5, 3.12.3, 3.13.1, 3.13.5, 3.8.6, 3.9.5, 3.9.6 |
| `tkrplot` | 0.0-30 |
| `tkWidgets` | 1.88.0 |
| `tldr` | 1.2.2 |
| `tlparse` | 0.3.37 |
| `tm` | 0.7-16 |
| `tmap` | 4.2 |
| `tmaptools` | 3.3 |
| `TMB` | 1.9.18 |
| `tmle` | 2.1.1 |
| `tmvnsim` | 1.0-2 |
| `tmvtnorm` | 1.7 |
| `Togl` | 2.0 |
| `tokenizers` | 0.15.2, 0.15.2, 0.19.1, 0.19.1, 0.3.0 |
| `toml` | 0.10.2 |
| `TOML-Fortran` | 0.4.2 |
| `tomli` | 2.2.1 |
| `tomli-w` | 1.2.0 |
| `tomlkit` | 0.12.3, 0.13.2, 0.13.3 |
| `TomoPy` | 1.11.0 |
| `toolz` | 0.12.0, 0.12.1 |
| `topicmodels` | 0.2-17 |
| `toposort` | 1.10 |
| `torchvision` | 0.16.0, 0.21.0, 0.21.0 |
| `tornado` | 6.3.2, 6.4, 6.4.1 |
| `tqdm` | 4.61.2, 4.62.3, 4.64.0, 4.66.1, 4.66.2, 4.66.5, 4.67.1 |
| `traitlets` | 5.13.0 |
| `TrajectoryUtils` | 1.18.0 |
| `TraMineR` | 2.2-12 |
| `Transformers` | 4.39.3, 4.44.0 |
| `transformers` | 4.39.3, 4.44.0 |
| `transitions` | 0.9.2 |
| `tree` | 1.0-45 |
| `Tree::DAG_Node` | 1.32, 1.35 |
| `treeio` | 1.34.0 |
| `TreeSummarizedExperiment` | 2.18.0 |
| `TRF` | 4.09.1 |
| `triebeard` | 0.4.1 |
| `trimAl` | 1.4.1 |
| `trimcluster` | 0.2-0 |
| `Trimmomatic` | 0.39-Java-11 |
| `Trinity` | 2.15.1 |
| `tripack` | 1.3-9.3 |
| `TRIQS` | 3.1.1, 3.2.0 |
| `TRIQS-cthyb` | 3.2.1 |
| `TRIQS-tprf` | 3.2.1 |
| `trogon` | 0.5.0 |
| `trove-classifiers` | 2025.2.18.16, 2025.5.9.12 |
| `TruncatedNormal` | 2.3 |
| `truncnorm` | 1.0-9 |
| `trust` | 0.1-8 |
| `truststore` | 0.9.2 |
| `Try::Tiny` | 0.32 |
| `tseries` | 0.10-58 |
| `tseriesChaos` | 0.1-13.1 |
| `tsna` | 0.3.6 |
| `tsne` | 0.1-3.1 |
| `TSP` | 1.2-5 |
| `TTR` | 0.24.4 |
| `tuneR` | 1.4.7 |
| `twang` | 2.6.1 |
| `tweedie` | 2.3.5 |
| `tweenr` | 2.0.3 |
| `TxDb.Hsapiens.UCSC.hg19.knownGene` | 3.22.1 |
| `TxDb.Mmusculus.UCSC.mm10.knownGene` | 3.10.0 |
| `txdbmaker` | 1.6.0 |
| `tximport` | 1.38.0 |
| `Type::Tiny` | 2.006000, 2.008002 |
| `typer` | 0.12.5, 0.4.1 |
| `Types::Serialiser` | 1.01 |
| `typing-extensions` | 3.7.4.3, 4.10.0, 4.11.0, 4.12.2, 4.14.1, 4.9.0 |
| `typing_extensions` | 4.12.2, 4.14.0, 4.4.0 |
| `tzdata` | 2025.2 |
| `tzdb` | 0.5.0 |

## U

| Modulename | Versions |
| :--- | :--- |
| `ubiquerg` | 0.7.0, 0.8.0 |
| `UCC` | 1.0.0, 1.1.0, 1.2.0, 1.3.0, 1.4.4 |
| `UCC-CUDA` | 1.3.0-CUDA-12.8.0 |
| `UCell` | 2.14.0 |
| `ucminf` | 1.2.2 |
| `UCSC.utils` | 1.6.0 |
| `UCX` | 1.10.0, 1.11.2, 1.12.1, 1.13.1, 1.14.1, 1.15.0, 1.16.0, 1.18.0, 1.19.0, 1.9.0, 1.9.0-CUDA-11.1.1 |
| `UCX-CUDA` | 1.11.2-CUDA-11.4.1, 1.12.1-CUDA-11.7.0, 1.14.1-CUDA-12.1.1, 1.16.0-CUDA-12.6.0, 1.18.0-CUDA-12.8.0 |
| `udpipe` | 0.8.12 |
| `UDUNITS` | 2.2.28 |
| `ujson` | 5.10.0 |
| `umap` | 0.2.10.0 |
| `umap-learn` | 0.5.5, 0.5.5 |
| `unbalanced` | 2.0 |
| `unearth` | 0.17.2 |
| `Unicode::EastAsianWidth` | 12.0 |
| `Unicode::LineBreak` | 2019.001 |
| `Unidecode` | 1.3.6 |
| `unidist` | 0.7.2 |
| `unifdef` | 2.12 |
| `unikn` | 1.0.0 |
| `uniqueAtomMat` | 0.1-3-2 |
| `units` | 1.0-0 |
| `unittest-xml-reporting` | 3.1.0 |
| `UNIVERSAL::moniker` | 0.08 |
| `universalmotif` | 1.28.0 |
| `Unix::Processors` | 2.046 |
| `unmarked` | 1.5.1 |
| `UnZip` | 6.0 |
| `UpSetR` | 1.4.0 |
| `urca` | 1.3-4 |
| `URI` | 5.31, 5.32 |
| `url-normalize` | 1.4.3 |
| `urlchecker` | 1.0.1 |
| `urllib3` | 2.1.0, 2.4.0, 2.5.0 |
| `urltools` | 1.7.3.1 |
| `uroot` | 2.1-3 |
| `USEARCH` | 11.0.667-i86linux32 |
| `usethis` | 3.2.1 |
| `utf8` | 1.2.6 |
| `utf8proc` | 2.10.0, 2.6.1, 2.7.0, 2.8.0, 2.9.0 |
| `util-linux` | 2.36, 2.37, 2.38, 2.38.1, 2.39, 2.40, 2.41 |
| `uuid` | 1.2-1 |
| `uwot` | 0.2.3, 0.2.4 |

## V

| Modulename | Versions |
| :--- | :--- |
| `V8` | 8.0.1 |
| `Valgrind` | 3.20.0, 3.25.1, 3.26.0 |
| `varhandle` | 2.0.6 |
| `Variable::Magic` | 0.64 |
| `variancePartition` | 1.40.0 |
| `VariantAnnotation` | 1.56.0 |
| `VarScan` | 2.4.4-Java-11 |
| `VASP` | 5.4.4_pl2, 6.3.0, 6.5.1 |
| `VBZ-Compression` | 1.0.3 |
| `vcd` | 1.4-13 |
| `vcflib` | 1.0.2, 1.0.9-R-4.3.2 |
| `vcfR` | 1.15.0 |
| `VCFtools` | 0.1.16 |
| `vctrs` | 0.6.5 |
| `vegan` | 2.7-2 |
| `velocyto` | 0.17.17, 0.17.17 |
| `Velvet` | 1.2.10-mt-kmer_191 |
| `venn` | 1.12 |
| `VennDiagram` | 1.7.3 |
| `VEP` | 107 |
| `version` | 0.9933 |
| `versioneer` | 0.29 |
| `versioningit` | 3.1.2 |
| `VGAM` | 1.1-13 |
| `VIM` | 6.2.6 |
| `VineCopula` | 2.6.1 |
| `vioplot` | 0.5.1 |
| `vipor` | 0.4.7 |
| `viridis` | 0.6.5 |
| `viridisLite` | 0.4.2 |
| `virtualenv` | 20.23.1, 20.24.6, 20.26.2, 20.29.2, 20.29.2, 20.32.0, 20.32.0 |
| `visdat` | 0.6.0 |
| `visNetwork` | 2.1.4 |
| `VMD` | 1.9.4a57 |
| `Voro++` | 0.4.6 |
| `vroom` | 1.6.6 |
| `VSCode` | 1.104.1, 1.92.2, 1.96.4, 1.97.1 |
| `vsn` | 3.78.0 |
| `VSURF` | 1.2.1 |
| `VTK` | 9.0.1, 9.1.0, 9.2.2, 9.2.6, 9.3.0 |

## W

| Modulename | Versions |
| :--- | :--- |
| `waiter` | 0.2.5.1 |
| `waldo` | 0.6.2 |
| `Wannier90` | 3.1.0 |
| `Want` | 0.29 |
| `warp` | 0.2.1 |
| `wateRmelon` | 2.16.0 |
| `waveslim` | 1.8.5 |
| `Wayland` | 1.20.0, 1.22.0, 1.23.0, 1.23.92, 1.24.0 |
| `WCSim` | 1.6-virt |
| `wcwidth` | 0.2.13 |
| `wdm` | 0.2.6 |
| `webencodings` | 0.5.1 |
| `webshot` | 0.5.5 |
| `websocket-client` | 1.5.1 |
| `websocket_client` | 1.8.0 |
| `webutils` | 1.2.2 |
| `weights` | 1.1.2 |
| `WeightSVM` | 1.7-16 |
| `wellknown` | 0.7.4 |
| `Werkzeug` | 2.3.7, 3.1.3 |
| `werkzeug` | 2.3.7, 3.0.4 |
| `WFA2` | 2.3.4 |
| `wgbs_tools` | v0.2.2 |
| `WGCNA` | 1.73 |
| `wget` | 1.24.5 |
| `WhatsHap` | 2.2, 2.6 |
| `whatshap` | 2.2, 2.6 |
| `wheel` | 0.45.1 |
| `whisker` | 0.4.1 |
| `Whisper` | 1.3.0 |
| `Whisper-gui` | 0.1.0 |
| `widgetframe` | 0.3.1 |
| `widgetsnbextension` | 4.0.13 |
| `widgetTools` | 1.88.0 |
| `WikidataQueryServiceR` | 1.0.0 |
| `WikidataR` | 2.3.3 |
| `WikipediR` | 1.7.1 |
| `wikitaxa` | 0.4.0 |
| `withr` | 3.0.2 |
| `wk` | 0.9.4 |
| `word2vec` | 0.4.0 |
| `wordcloud` | 2.6 |
| `worrms` | 0.4.3 |
| `wrapt` | 1.15.0, 1.15.0, 1.16.0, 1.16.0 |
| `Wrench` | 1.28.0 |
| `writexl` | 1.5.4 |
| `WriteXLS` | 6.8.0 |
| `wtdbg2` | 2.5 |
| `wurlitzer` | 3.1.1 |
| `WWW::RobotRules` | 6.02 |
| `wxWidgets` | 3.2.1 |

## X

| Modulename | Versions |
| :--- | :--- |
| `X11` | 20201008, 20210518, 20210802, 20220504, 20221110, 20230603, 20231019, 20240607, 20250521, 20250608 |
| `x264` | 20201026, 20210414, 20210613, 20220620, 20230226, 20231019, 20240513, 20250619, 20250831 |
| `x265` | 3.3, 3.5, 3.6, 4.1 |
| `xarray` | 2022.9.0, 2023.9.0 |
| `XBeach` | 20230831, 20231031 |
| `XBRL` | 0.99.19.1 |
| `xcms` | 4.8.0 |
| `XCrySDen` | 1.6.2 |
| `Xerces-C++` | 3.2.4, 3.2.5, 3.3.0 |
| `xfun` | 0.53, 0.54 |
| `xgboost` | 1.7.11.1 |
| `xlrd` | 2.0.1, 2.0.2 |
| `xlsx` | 0.6.5 |
| `xlsxjars` | 0.9.0 |
| `XML` | 3.99-0.19 |
| `XML-Compile` | 1.63 |
| `XML-LibXML` | 2.0207, 2.0208, 2.0210 |
| `xml2` | 1.4.0, 1.4.1 |
| `XML::Bare` | 0.53 |
| `XML::Compile` | 1.63 |
| `XML::Compile::Cache` | 1.06 |
| `XML::Compile::SOAP` | 3.28 |
| `XML::Compile::WSDL11` | 3.08 |
| `XML::DOM` | 1.46 |
| `XML::Filter::BufferText` | 1.01 |
| `XML::LibXML` | 2.0210 |
| `XML::LibXML::Simple` | 1.01 |
| `XML::NamespaceSupport` | 1.12 |
| `XML::Parser` | 2.47 |
| `XML::RegExp` | 0.04 |
| `XML::SAX` | 1.02 |
| `XML::SAX::Base` | 1.09 |
| `XML::SAX::Expat` | 0.51 |
| `XML::SAX::Writer` | 0.57 |
| `XML::Simple` | 2.25 |
| `XML::Tiny` | 2.07 |
| `XML::Twig` | 3.52, 3.54 |
| `XML::Writer` | 0.900 |
| `XML::XPath` | 1.48 |
| `xmlf90` | 1.6.3 |
| `xmlschema` | 2.5.0 |
| `xopen` | 1.0.1, 1.7.0, 1.9.0, 2.0.2 |
| `xorg-macros` | 1.19.2, 1.19.3, 1.20.0, 1.20.1, 1.20.2 |
| `xprop` | 1.2.5 |
| `xproto` | 7.0.31 |
| `XSLoader` | 0.24 |
| `xtable` | 1.8-4 |
| `xtb` | 6.6.1 |
| `xTea` | v0.1.0-long |
| `xts` | 0.14.1 |
| `XVector` | 0.50.0 |
| `Xvfb` | 1.20.13, 21.1.18, 21.1.3, 21.1.6, 21.1.8 |
| `xxd` | 8.2.4220, 9.0.2112, 9.1.0307 |
| `xxHash` | 0.8.2 |
| `xyzservices` | 2024.4.0 |
| `XZ` | 5.2.5, 5.2.7, 5.4.2, 5.4.4, 5.4.5, 5.6.3, 5.8.1 |

## Y

| Modulename | Versions |
| :--- | :--- |
| `yacman` | 0.9.3 |
| `yaImpute` | 1.0-34.1 |
| `YAML` | 1.31 |
| `yaml` | 2.3.10 |
| `yaml-cpp` | 0.7.0 |
| `YAML::PP` | v0.39.0 |
| `YAML::Tiny` | 1.74, 1.76 |
| `yarl` | 1.15.4, 1.9.2 |
| `Yasm` | 1.3.0 |
| `yte` | 1.5.4, 1.5.5 |
| `yulab.utils` | 0.2.1 |

## Z

| Modulename | Versions |
| :--- | :--- |
| `Z3` | 4.10.2, 4.12.2, 4.13.0, 4.13.4, 4.15.1, 4.8.10, 4.8.11, 4.8.12 |
| `z3-solver` | 4.13.0.0 |
| `z3_solver` | 4.13.4.0, 4.15.1.0 |
| `zCompositions` | 1.5.0-5 |
| `zeallot` | 0.2.0 |
| `zellkonverter` | 1.20.0 |
| `ZeroMQ` | 4.3.3, 4.3.4, 4.3.5 |
| `zfp` | 1.0.0 |
| `zict` | 3.0.0 |
| `zigg` | 0.0.2 |
| `Zip` | 3.0 |
| `zip` | 2.3.3 |
| `zipfile36` | 0.1.3 |
| `zipp` | 3.17.0, 3.21.0, 3.23.0 |
| `zlib` | 1.2.11, 1.2.12, 1.2.13, 1.3.1, 2.3.2 |
| `zlib-ng` | 0.4.1, 0.5.0, 2.0.7, 2.2.1 |
| `Zoltan` | 3.901 |
| `zoo` | 1.8-14 |
| `Zopfli` | 1.0.3 |
| `zstandard` | 0.22.0, 0.23.0 |
| `zstd` | 1.4.5, 1.4.9, 1.5.0, 1.5.2, 1.5.5, 1.5.6, 1.5.7, 1.5.7 |
| `zUMIs` | 2.9.7-R-4.3.2 |



---

**Last updated:** 2026-08-12

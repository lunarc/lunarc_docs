...
SSHFS is a network filesystem that relies on the SSH service to transfer files. Once a volume is connected, the files in the remote system are available on the user's computer as if they were local files. There is a wide variety of software available for [Windows, macOS or Linux](http://pig.made-it.com/sshfs.html) able to connect a network volume via SSHFS. Most graphical user interfaces for connecting SSHFS volumes, mainly on Windows or macOS, fail to comply with LUNARC's mandatory [two-factor authentication](../../getting_started/login_howto.md). This guide describes a method to connect a remote SSHFS volume using a Linux terminal, even with LUNARC's second factor authentication.
